-- ============================================================
-- SISTEMA CSF OPERATIVA — 01_schema.sql  v1.1
-- CORRECCIONES:
--   S1 — UNIQUE constraint en servicios para upsert correcto
--   S3 — Nueva tabla config_cuartel para denominadores IDFI
-- ============================================================
...
-- ── SERVICIOS ────────────────────────────────────────────────
-- S1: UNIQUE compuesto para que el upsert de importación Excel funcione correctamente
CREATE TABLE servicios (
  ...
  -- S1: constraint para upsert correcto al reimportar Excel
  UNIQUE(cuartel_id, fecha, tipo_servicio, hora_inicio)
);
...
CREATE TABLE personas_registradas (
  ...
  como_inicio             TEXT CHECK (como_inicio IN (
                            'control_identidad_preventivo','control_identidad_investigativo',
                            'control_migratorio','control_vehicular','patrullaje_flagrancia')),
  ...
  nna_vinculo_adulto      TEXT CHECK (nna_vinculo_adulto IN ('padre_madre','familiar','sin_vinculo',NULL)),
  nna_derivacion          TEXT,
  adulto_imputado_id      UUID REFERENCES personas_registradas(id),
  vinculacion_inst        TEXT CHECK (vinculacion_inst IN ('no','activo','exmiembro',NULL)),
...
-- ── S3: TABLA CONFIG_CUARTEL (denominadores IDFI) ────────────
-- Persiste configuración por cuartel: meta UF, objetivos internacionales, etc.
CREATE TABLE config_cuartel (
  id                        UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  cuartel_id                UUID REFERENCES cuarteles(id) ON DELETE CASCADE UNIQUE,
  meta_uf_mensual           DECIMAL(10,2) DEFAULT 50,
  objetivos_internacionales INTEGER DEFAULT 0,
...
-- Índice extra para la query de última visita por punto (B1)
CREATE INDEX idx_visitas_punto_fecha     ON visitas_puntos(punto_id, fecha DESC);
