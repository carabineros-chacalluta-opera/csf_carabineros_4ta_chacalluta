...
-- ── CONFIG_CUARTEL — cada cuartel gestiona su propia config ──
ALTER TABLE config_cuartel ENABLE ROW LEVEL SECURITY;

CREATE POLICY config_read ON config_cuartel
  FOR SELECT USING (cuartel_id = get_user_cuartel());

CREATE POLICY config_write ON config_cuartel
  FOR ALL USING (
    cuartel_id = get_user_cuartel()
    AND get_user_rol() IN ('comisario','administrador')
  );
