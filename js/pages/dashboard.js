// ============================================================
// SISTEMA CSF OPERATIVA — dashboard.js  v2.1
// CORRECCIONES v2.1:
//   FIX-C05 — dfp04 ahora es proporcional (completados/total*100) en lugar de binario
//   FIX-Q07 — _dashModoMulti NO se resetea en cada render; preserva toggle manual del usuario
//   FIX-Q08 — Promise.all() del dashboard envuelto en try/catch con mensaje de error en pantalla
// ============================================================

// Estado de navegacion del dashboard
let _dashMes            = new Date().getMonth() + 1
let _dashAnio           = new Date().getFullYear()
let _dashModoMulti      = false
let _dashModoMultiInit  = false   // FIX-Q07: flag para no resetear el toggle manual

async function renderDashboard() {
  _dashMes  = new Date().getMonth() + 1
  _dashAnio = new Date().getFullYear()
  // FIX-Q07: solo inicializar en la primera llamada, no sobreescribir toggle manual
  if (!_dashModoMultiInit) {
    _dashModoMulti     = APP.esValidador()
    _dashModoMultiInit = true
  }
...
async function _renderDashboardConMes(mes, anio) {
  ...
  // FIX-Q08: try/catch en el bloque principal del dashboard
  try {
    const cuartelActivo = APP.cuartelActivo()
    const cuartelId     = cuartelActivo?.id
    const puedeVerTodos = APP.esAdministrador() || APP.esComisario() || APP.esValidador()
    ...
    const { data: svcsBase, error: svcsErr } = await svcsQuery
    ...
    // FIX-Q08: cargar datos en paralelo con manejo de error individual
    let controles, incautaciones, personas, csf_activa, alertas, reportes_pend, puntos, visitasMes
    try {
      const results = await Promise.all([
        ...
        (() => {
          let q = APP.sb.from('csf_mensual').select('*').eq('estado','publicada')
            .gte('fecha_vigencia_inicio', ini).lte('fecha_vigencia_fin', fin)
      ...
      console.error('[Dashboard] Error en consultas paralelas:', parallelErr)
      el('pantalla-dashboard').innerHTML = `
        <div class="container">
          <div class="card" style="padding:2rem;text-align:center;color:var(--rojo)">
            ⚠️ Error al cargar algunos datos del dashboard.<br>
            <small style="color:var(--muted)">${parallelErr?.message || 'Error de red'}</small><br>
            <button class="btn btn-secundario" style="margin-top:1rem" onclick="renderDashboard()">Reintentar</button>
          </div>
        </div>`
      return
    }
    ...
    const idfiData = await calcularIDFIDashboard({
      ...
      completados, servicios, ini, hoy: fechaRef,
    })
    ...
    el('pantalla-dashboard').innerHTML = `
      <div class="container">
        <div class="card" style="padding:2rem;text-align:center;color:var(--rojo)">
          ⚠️ Error al cargar el dashboard.<br>
          <small style="color:var(--muted)">${e.message}</small><br>
          <button class="btn btn-secundario" style="margin-top:1rem" onclick="renderDashboard()">Reintentar</button>
        </div>
      </div>`
...
// Vista multi-cuartel comparativa
async function _renderDashboardMultiCuartel(mes, anio, esHistorico) {
  ...
  el('pantalla-dashboard').innerHTML = `
    <div class="container">
      <div class="dash-nav-mes">
        <button class="btn btn-ghost btn-sm" onclick="dashNavMes(-1)">◀ ${MESES_ES[(mes - 2 + 12) % 12]}</button>
        <div class="dash-nav-label ${esHistorico ? 'dash-nav-historico' : ''}">
          ${esHistorico ? '🕓 ' : '📅 '}<strong>${mesLabel}</strong>
        </div>
        <button class="btn btn-ghost btn-sm" ${!esHistorico ? 'disabled style="opacity:.35;cursor:default"' : ''}
          onclick="dashNavMes(1)">${MESES_ES[mes % 12]} ▶</button>
        ${APP.esValidador() ? '' : `
        <button class="btn btn-primario btn-sm" onclick="dashToggleModoMulti()" style="margin-left:auto">← Mi cuartel</button>`}
      </div>
      <div class="dash-header">
        <div>
      ...
      ${esHistorico ? `<div style="background:var(--azul-cl,#EBF3FB);border:1px solid var(--azul,#2980B9);border-radius:8px;padding:.55rem .9rem;margin-bottom:.85rem;font-size:.8rem;color:var(--azul,#1a5f8a)">📋 Datos históricos de <strong>${mesLabel}</strong></div>` : ''}
      <div class="kpi-grid" style="margin-bottom:1rem">
        ...
        ${kpiCard('Detenidos totales', datosPorCuartel.reduce((a,d) => a+d.totalDetenidos,0), mesLabel, 'ok')}
        ${kpiCard('UF totales red', datosPorCuartel.reduce((a,d) => a+d.totalUF,0).toFixed(1), mesLabel, 'neutral')}
      </div>
      <div class="card" style="padding:0;overflow:hidden;margin-bottom:1rem">
        <div class="tabla-header" style="padding:.6rem 1rem"><span>Comparativa IDFI por cuartel — ${mesLabel}</span></div>
            ...
            <tbody>
              ${datosPorCuartel.map((d,i) => {
                ...
                return `<tr style="${rowBg};border-bottom:1px solid var(--border)">
                  ...
                  <td style="padding:.4rem .75rem;font-weight:600">${d.cuartel.nombre.replace(' (F)','')}</td>
                  <td style="padding:.4rem .75rem;text-align:center"><span style="font-size:1.1rem;font-weight:700;color:${col}">${score}</span>${lbl?`<div style="font-size:.6rem;color:${col}">${lbl.label}</div>`:''}</td>
                  ...
                  <td style="padding:.4rem .75rem;text-align:center"><span style="color:var(--verde)">${d.completados}</span>${d.pendientes>0?`<span style="color:var(--amarillo);font-size:.7rem"> +${d.pendientes}p</span>`:''}</td>
                  <td style="padding:.4rem .75rem;text-align:center">${d.totControles.toLocaleString('es-CL')}</td>
                  <td style="padding:.4rem .75rem;text-align:center;font-weight:${d.totalDetenidos>0?'700':'400'};color:${d.totalDetenidos>0?'var(--verde)':'inherit'}">${d.totalDetenidos}</td>
                  <td style="padding:.4rem .75rem;text-align:center">${d.totalUF.toFixed(1)}</td>
                  <td style="padding:.4rem .75rem;text-align:center">${d.tieneCsf?'<span style="color:var(--verde);font-weight:700;font-size:1rem">✓</span>':'<span style="color:var(--rojo);font-size:.7rem">Sin CSF</span>'}</td>
                </tr>`
              }).join('')}
            </tbody>
          </table>
      ...
      <div class="card">
        ...
        <div style="display:flex;flex-direction:column;gap:.65rem;margin-top:.85rem">
          ${datosPorCuartel.map((d,i) => {
            ...
            const col   = lbl?.color || 'var(--muted)'
            return `<div style="display:grid;grid-template-columns:170px 1fr 48px;align-items:center;gap:.5rem">
...
async function _datosCuartelParaComparativa(c, ini, fin, fechaRef) {
  try {
    const { data: svcs } = await APP.sb.from('servicios')
      .select('id,estado').eq('cuartel_id', c.id).gte('fecha', ini).lte('fecha', fin)
    const svcIds      = (svcs || []).map(s => s.id)
    const completados = (svcs || []).filter(s => s.estado === 'completado')
    const pendientes  = (svcs || []).filter(s => s.estado === 'pendiente').length

    const [
      { data: controles }, { data: incautaciones }, { data: personas },
      { data: puntos }, { data: visitasMes }, { data: csf_activa },
    ] = await Promise.all([
      ...
      svcIds.length ? APP.sb.from('visitas_puntos').select('punto_id,fecha,servicio_id').gte('fecha', ini).lte('fecha', fin).in('servicio_id', svcIds) : Promise.resolve({ data: [] }),
      APP.sb.from('csf_mensual').select('id').eq('cuartel_id', c.id).eq('estado', 'publicada').gte('fecha_vigencia_inicio', ini).lte('fecha_vigencia_fin', fin).limit(1),
    ])
    ...
    const idfi = await calcularIDFIDashboard({
      ...
      completados, servicios: svcs||[], ini, hoy: fechaRef,
    })

    const totControles   = (controles||[]).reduce((a,ct) => a+(ct.identidad_preventivos||0)+(ct.identidad_investigativos||0)+(ct.migratorios||0)+(ct.vehiculares||0),0)
    const totalDetenidos = (personas||[]).filter(p => p.tipo_resultado === 'detencion').length
    const totalUF        = (incautaciones||[]).reduce((a,i) => a+(i.valor_uf||0),0)
...
function bloqueIDFI(data) {
  if (!data) return `<div class="card" style="text-align:center;padding:1.5rem;color:var(--muted)">Sin puntos territoriales registrados para calcular IDFI</div>`
  const { idfi, dfp, dfo, label } = data
  const barColor = label.color
  return `<div class="card" style="border-left:4px solid ${barColor}">
    <div class="sec-titulo" style="margin-bottom:1rem">Índice de Desempeño Fronterizo Integral (IDFI)</div>
...
function seguimientoMiniCSF(visitasOrdenadas, hoy) {
  if (!visitasOrdenadas.length) return `<div style="font-size:.78rem;color:var(--muted);padding:.5rem 0">Sin visitas programadas en esta CSF</div>`
  ...
  const rows = puntosIds.map(pid => {
    ...
    return `<div class="csf-seguimiento-row">
  ...
  return `<div class="csf-seguimiento">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:.5rem">
      <div style="font-size:.76rem;font-weight:600">Seguimiento por punto</div>
      <div style="font-size:.76rem;font-weight:700;color:${colorGlobal}">Global: ${pctGlobal}%</div>
    </div>
    ${rows}
  </div>`
}
...
function resumenVisitasPorTipo(visitasMes, puntos) {
  const visitadosHitos = new Set(visitasMes.filter(v => puntos.find(p => p.id===v.punto_id&&p.tipo==='hito')).map(v=>v.punto_id))
  const visitadosPnh   = new Set(visitasMes.filter(v => puntos.find(p => p.id===v.punto_id&&p.tipo==='pnh')).map(v=>v.punto_id))
  const visitadosSie   = new Set(visitasMes.filter(v => puntos.find(p => p.id===v.punto_id&&p.tipo==='sie')).map(v=>v.punto_id))
  ...
  const pct = n => n >= 80 ? 'var(--verde)' : n >= 50 ? 'var(--amarillo)' : 'var(--rojo)'
  ...
  return `<div class="tipo-visitas-grid">
    <div class="tipo-visita-card tipo-hito">
      ...
      <div style="font-size:.85rem;font-weight:600;color:${pct(pctHito)}">${pctHito}% cobertura</div>
    </div>
    <div class="tipo-visita-card tipo-pnh">
      ...
      <div style="font-size:.85rem;font-weight:600;color:${pct(pctPnh)}">${pctPnh}% cobertura</div>
    </div>
    <div class="tipo-visita-card tipo-sie">
      ...
      <div style="font-size:.85rem;font-weight:600;color:${pct(pctSie)}">${pctSie}% cobertura</div>
    </div>
  </div>`
}

// FIX-C05: dfp04 proporcional — recibe servicios (total) ademas de completados
async function calcularIDFIDashboard({ cuartelId, puntos, visitasMes, personas,
  incautaciones, controles, reportesPend, completados, servicios, ini, hoy }) {
  ...
  let cfg = null
  if (cuartelId) {
    const { data, error } = await APP.sb.from('config_cuartel').select('*').eq('cuartel_id', cuartelId).maybeSingle()
    cfg = (!error && data) ? data : null
  }
  ...
  // FIX-C05: proporcion de servicios completados vs total del mes
  const totalServicios = (servicios||[]).length
  const dfp04 = totalServicios > 0 ? (completados.length / totalServicios) * 100 : 0
  const dfp05 = reportesPend === 0 ? 100 : Math.max(0, 100 - reportesPend * 20)

  const dfp = (
    dfp01*CSF_CONFIG.DFP_PESOS.dfp01 + dfp02*CSF_CONFIG.DFP_PESOS.dfp02 +
    dfp03*CSF_CONFIG.DFP_PESOS.dfp03 + dfp04*CSF_CONFIG.DFP_PESOS.dfp04 +
  ...
  const dfo = (
    dfo01*CSF_CONFIG.DFO_PESOS.dfo01 + dfo02*CSF_CONFIG.DFO_PESOS.dfo02 +
    dfo03*CSF_CONFIG.DFO_PESOS.dfo03 + dfo04*CSF_CONFIG.DFO_PESOS.dfo04 +
    dfo05*CSF_CONFIG.DFO_PESOS.dfo05 + dfo06*CSF_CONFIG.DFO_PESOS.dfo06
  )

  const idfi = dfp*CSF_CONFIG.IDFI_PESOS.dfp + dfo*CSF_CONFIG.IDFI_PESOS.dfo

  return {
    idfi: Math.round(idfi), dfp: Math.round(dfp), dfo: Math.round(dfo),
