...
function htmlFormServicio(svc, puntos, controles, visitas, yaCompletado) {
  ...
  return `
    ...
    <!-- PASO 0: Responsable + Personal + KM -->
    ...
    <!-- PERSONAL P.N.S / P.N.I -->
    <div class="form-seccion">
      <div class="form-seccion-titulo">Personal del servicio</div>
      <div class="controles-grid">
        ${campoNumero('svc-pns', 'Personal Nombramiento Supremo (P.N.S)', svc.personal_pns||0)}
        ${campoNumero('svc-pni', 'Personal Nombramiento Institucional (P.N.I)', svc.personal_pni||0)}
        ${campoNumero('svc-vehiculos', 'Vehículos', svc.cantidad_vehiculos||0)}
      </div>
    </div>

    <!-- KM RECORRIDOS -->
    <div class="form-seccion">
      <div class="form-seccion-titulo">Kilómetros recorridos</div>
      <div class="controles-grid">
        ${campoNumero('km-motorizado',  'KM Motorizado',   svc.km_motorizado||0)}
        ${campoNumero('km-montado',     'KM Montado',      svc.km_montado||0)}
        ${campoNumero('km-infanteria',  'KM Infantería',   svc.km_infanteria||0)}
        ${campoNumero('km-maritimo',    'KM Veh. Marítimo',svc.km_maritimo||0)}
      </div>
      <div style="font-size:.78rem;color:var(--muted);margin-top:.4rem">
        Total: <strong id="km-total">${(svc.km_motorizado||0)+(svc.km_montado||0)+(svc.km_infanteria||0)+(svc.km_maritimo||0)} km</strong>
      </div>
      <script>
        document.querySelectorAll('#km-motorizado,#km-montado,#km-infanteria,#km-maritimo').forEach(i=>
          i.addEventListener('input',()=>{
            const t=['km-motorizado','km-montado','km-infanteria','km-maritimo']
              .reduce((s,id)=>s+(parseFloat(document.getElementById(id)?.value)||0),0)
            const el2=document.getElementById('km-total'); if(el2) el2.textContent=t.toFixed(1)+' km'
          })
        )
      <\/script>
    </div>

    <!-- COORDENADAS GPS DEL SERVICIO -->
    <div class="form-seccion" id="seccion-coords-servicio">
      <div class="form-seccion-titulo">Coordenadas GPS del recorrido</div>
      <div class="g2">
        <div class="campo">
          <label>Latitud</label>
          <input id="svc-lat" type="text" placeholder="Ej: -18.3123" value="${svc.latitud_recorrido||''}"/>
        </div>
        <div class="campo">
          <label>Longitud</label>
          <input id="svc-lon" type="text" placeholder="Ej: -70.1456" value="${svc.longitud_recorrido||''}"/>
        </div>
      </div>
      <button class="btn btn-secundario btn-sm" style="margin-top:.5rem" onclick="abrirMapaCoordenadas('svc-lat','svc-lon')">
        ¿No conoces las coordenadas? Seleccionar en mapa
      </button>
    </div>
    ...
    <!-- S7: Entrevista con policía extranjera -->
    <div class="form-seccion">
      <div class="form-seccion-titulo">S7 — ¿Se realizó entrevista con policía extranjera?</div>
      <div class="toggle-result">
        <button class="btn-toggle" id="btn-entrev-si" onclick="toggleSeccion('entrevista', true)">Sí</button>
        <button class="btn-toggle btn-toggle-active" id="btn-entrev-no" onclick="toggleSeccion('entrevista', false)">No</button>
      </div>
      <div id="seccion-entrevista" style="display:none">
        <div class="g2" style="margin-top:.75rem">
          <div class="campo">
            <label>Tipo de entrevista</label>
            <select id="entrev-tipo">
              <option value="PNP">PNP — Policía Nacional del Perú</option>
              <option value="PNB">PNB — Policía Boliviana</option>
            </select>
          </div>
          <div class="campo">
            <label>Lugar de encuentro</label>
            <select id="entrev-punto" onchange="autocompletarCoordsEntrevista(this.value)">
              <option value="">— Seleccionar punto —</option>
              ${_puntosDelCuartel.map(p=>`<option value="${p.id}" data-lat="${p.latitud||''}" data-lon="${p.longitud||''}">${p.nombre}</option>`).join('')}
            </select>
          </div>
          <div class="campo">
            <label>Hora inicio</label>
            <input type="time" id="entrev-hora-ini"/>
          </div>
          <div class="campo">
            <label>Hora término</label>
            <input type="time" id="entrev-hora-fin"/>
          </div>
          <div class="campo">
            <label>Grado del policía entrevistado</label>
            <input type="text" id="entrev-grado" placeholder="Ej: Alférez, Sargento 1ro..."/>
          </div>
          <div class="campo">
            <label>Nombre del policía entrevistado</label>
            <input type="text" id="entrev-nombre" placeholder="Nombre completo..."/>
          </div>
        </div>
        <div class="campo" style="margin-top:.5rem">
          <label>Temas tratados</label>
          <textarea id="entrev-temas" rows="2" placeholder="Ej: Contrabando, tráfico de migrantes, tráfico de drogas..."></textarea>
        </div>
        <div class="g2" style="margin-top:.5rem">
          <div class="campo">
            <label>KM recorridos para entrevista</label>
            <input type="number" id="entrev-km" min="0" step="0.1" placeholder="0"/>
          </div>
        </div>
        <div class="g2" style="margin-top:.5rem">
          <div class="campo">
            <label>Latitud lugar entrevista</label>
            <input type="text" id="entrev-lat" placeholder="Auto desde punto"/>
          </div>
          <div class="campo">
            <label>Longitud lugar entrevista</label>
            <input type="text" id="entrev-lon" placeholder="Auto desde punto"/>
          </div>
        </div>
        <button class="btn btn-secundario btn-sm" style="margin-top:.4rem" onclick="abrirMapaCoordenadas('entrev-lat','entrev-lon')">
          ¿No conoces las coordenadas? Seleccionar en mapa
        </button>
      </div>
      <div id="seccion-entrevista-no" style="display:none;margin-top:.5rem">
        <div class="campo">
          <label>¿Estaba agendada?</label>
          <select id="entrev-agendada">
            <option value="no">No estaba agendada</option>
            <option value="si">Sí estaba agendada</option>
          </select>
        </div>
        <div class="campo" style="margin-top:.5rem">
          <label>Motivo de no realización</label>
          <textarea id="entrev-motivo-no" rows="2" placeholder="Indicar motivo..."></textarea>
        </div>
      </div>
    </div>

    <!-- S8: Procedimiento de rescate -->
    <div class="form-seccion">
      <div class="form-seccion-titulo">S8 — ¿Se realizó procedimiento de rescate?</div>
      <div class="toggle-result">
        <button class="btn-toggle" id="btn-rescate-si" onclick="toggleSeccion('rescate', true)">Sí</button>
        <button class="btn-toggle btn-toggle-active" id="btn-rescate-no" onclick="toggleSeccion('rescate', false)">No</button>
      </div>
      <div id="seccion-rescate" style="display:none;margin-top:.75rem">
        <div class="campo">
          <label>Descripción del procedimiento</label>
          <textarea id="rescate-desc" rows="3" placeholder="Describir brevemente el procedimiento de rescate..."></textarea>
        </div>
        <div class="g2" style="margin-top:.5rem">
          <div class="campo">
            <label>Latitud lugar rescate</label>
            <input type="text" id="rescate-lat" placeholder="Ej: -18.3123"/>
          </div>
          <div class="campo">
            <label>Longitud lugar rescate</label>
            <input type="text" id="rescate-lon" placeholder="Ej: -70.1456"/>
          </div>
        </div>
        <button class="btn btn-secundario btn-sm" style="margin-top:.4rem" onclick="abrirMapaCoordenadas('rescate-lat','rescate-lon')">
          ¿No conoces las coordenadas? Seleccionar en mapa
        </button>
        <div style="margin-top:.75rem">
          <label style="font-size:.82rem;font-weight:600;color:var(--verde-osc)">Personal que participó en el rescate</label>
          <div style="display:flex;flex-wrap:wrap;gap:.75rem;margin-top:.5rem;font-size:.83rem">
            <label><input type="checkbox" id="rescate-cuartel" checked/> Cuartel (F)</label>
            <label><input type="checkbox" id="rescate-gope"/> GOPE</label>
            <label><input type="checkbox" id="rescate-bomberos"/> Bomberos</label>
            <label><input type="checkbox" id="rescate-ffaa"/> FF.AA.</label>
            <label><input type="checkbox" id="rescate-socorro"/> Socorro Andino</label>
            <label><input type="checkbox" id="rescate-civiles"/> Civiles</label>
          </div>
        </div>
        <div class="campo" style="margin-top:.75rem">
          <label>Medios utilizados</label>
          <textarea id="rescate-medios" rows="2" placeholder="Ej: Vehículo 4x4, camilla, botiquín de primeros auxilios..."></textarea>
        </div>
      </div>
    </div>
...
function toggleSeccion(nombre, mostrar) {
  ...
  // Entrevista: mostrar panel alternativo cuando es No
  if (nombre === 'entrevista') {
    const secNo = el('seccion-entrevista-no')
    if (secNo) secNo.style.display = mostrar ? 'none' : 'block'
  }
...
function inicializarMapa() {
  ...
  if (!cont || _mapaCoords) { if(_mapaCoords) { _mapaCoords.invalidateSize(); return } }
  _mapaCoords = L.map('mapa-coords-container').setView([-18.35, -70.15], 10)
  L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
    attribution: '© OpenStreetMap'
  }).addTo(_mapaCoords)
  // Agregar puntos del cuartel como referencia
  ...
  _mapaCoords.on('click', e => {
    ...
    const el2 = document.getElementById('coords-seleccionadas')
    if (el2) el2.textContent = `${e.latlng.lat.toFixed(6)}, ${e.latlng.lng.toFixed(6)}`
    const btn = document.getElementById('btn-confirmar-coords')
...
async function confirmarCoordenadas(idLat, idLon, puntoId) {
  ...
  // Ofrecer guardar en el punto si tiene id y coordenadas null
  if (puntoId) {
...
async function guardarServicio() {
  ...
  try {
    ...
    await APP.sb.from('servicios').update({
      ...
      // NUEVOS CAMPOS
      personal_pns:         parseInt(el('svc-pns')?.value)||0,
      personal_pni:         parseInt(el('svc-pni')?.value)||0,
      cantidad_vehiculos:   parseInt(el('svc-vehiculos')?.value)||0,
      km_motorizado:        parseFloat(el('km-motorizado')?.value)||0,
      km_montado:           parseFloat(el('km-montado')?.value)||0,
      km_infanteria:        parseFloat(el('km-infanteria')?.value)||0,
      km_maritimo:          parseFloat(el('km-maritimo')?.value)||0,
      km_total:             ['km-motorizado','km-montado','km-infanteria','km-maritimo']
                              .reduce((s,id)=>s+(parseFloat(el(id)?.value)||0),0),
      latitud_recorrido:    parseFloat(el('svc-lat')?.value)||null,
      longitud_recorrido:   parseFloat(el('svc-lon')?.value)||null,
    }).eq('id', svcId)
...
for (const p of persItems) {
  ...
  if (esInterpol) await APP.sb.from('alertas').insert({
    ...
    // S7 — Entrevista
    setProgreso(95, 'Guardando S7: entrevista...')
    const entrevSi = el('btn-entrev-si')?.classList.contains('btn-toggle-active')
    if (entrevSi) {
      await APP.sb.from('entrevistas_servicio').insert({
        servicio_id:      svcId,
        cuartel_id:       _servicioActual.cuartel_id,
        fecha:            _servicioActual.fecha,
        tipo:             el('entrev-tipo')?.value,
        punto_id:         el('entrev-punto')?.value || null,
        hora_inicio:      el('entrev-hora-ini')?.value || null,
        hora_termino:     el('entrev-hora-fin')?.value || null,
        grado_policia:    el('entrev-grado')?.value?.trim() || null,
        nombre_policia:   el('entrev-nombre')?.value?.trim() || null,
        temas_tratados:   el('entrev-temas')?.value?.trim() || null,
        km_recorridos:    parseFloat(el('entrev-km')?.value)||0,
        latitud:          parseFloat(el('entrev-lat')?.value)||null,
        longitud:         parseFloat(el('entrev-lon')?.value)||null,
        realizada:        true,
        agendada:         null,
        motivo_no:        null,
      })
    } else {
      const agendada = el('entrev-agendada')?.value === 'si'
      if (agendada) {
        await APP.sb.from('entrevistas_servicio').insert({
          servicio_id:  svcId,
          cuartel_id:   _servicioActual.cuartel_id,
          ...
          realizada:    false,
          agendada:     true,
          motivo_no:    el('entrev-motivo-no')?.value?.trim() || null,
        })
      }
    }

    // S8 — Rescate
    setProgreso(97, 'Guardando S8: rescate...')
    const rescateSi = el('btn-rescate-si')?.classList.contains('btn-toggle-active')
    if (rescateSi) {
      await APP.sb.from('rescates_servicio').insert({
        servicio_id:     svcId,
        cuartel_id:      _servicioActual.cuartel_id,
        fecha:           _servicioActual.fecha,
        descripcion:     el('rescate-desc')?.value?.trim() || null,
        latitud:         parseFloat(el('rescate-lat')?.value)||null,
        longitud:        parseFloat(el('rescate-lon')?.value)||null,
        personal_cuartel: el('rescate-cuartel')?.checked||false,
        personal_gope:    el('rescate-gope')?.checked||false,
        personal_bomberos:el('rescate-bomberos')?.checked||false,
        personal_ffaa:    el('rescate-ffaa')?.checked||false,
        personal_socorro: el('rescate-socorro')?.checked||false,
        personal_civiles: el('rescate-civiles')?.checked||false,
        medios_utilizados:el('rescate-medios')?.value?.trim()||null,
      })
    }
    setTimeout(() => {
