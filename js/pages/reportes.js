// ============================================================
// SISTEMA CSF OPERATIVA — reportes.js  v2.2
// FIX-R1: cuartelFilt usa null en vez de APP.cuartelActivo()
//          Admin/Comisario sin cuartel ve todos los cuarteles.
// FIX-R2: cuarteles(nombre,codigo) → cuarteles(nombre,comuna)
//          La columna codigo no existe en la tabla cuarteles.
// ============================================================
...
async function consultarReportes() {
  ...
  try {
    ...
    const [
      ...
      svcIds.length ? APP.sb.from('visitas_puntos').select('servicio_id').in('servicio_id', svcIds) : Promise.resolve({ data: [] }),
      svcIds.length ? APP.sb.from('observaciones_intel').select('servicio_id').in('servicio_id', svcIds) : Promise.resolve({ data: [] }),
      svcIds.length ? APP.sb.from('incautaciones').select('servicio_id').in('servicio_id', svcIds) : Promise.resolve({ data: [] }),
    ])

    // Mapas de conteo por servicio (reemplaza los embedded counts que pueden fallar sin FK)
    const visitasCount = {};    (visitasRep   ||[]).forEach(v => { visitasCount[v.servicio_id]  = (visitasCount[v.servicio_id]  ||0)+1 })
    const intelCount  = {};     (obsIntelRep  ||[]).forEach(o => { intelCount[o.servicio_id]    = (intelCount[o.servicio_id]    ||0)+1 })
    const incautCount = {};     (incautRep    ||[]).forEach(i => { incautCount[i.servicio_id]   = (incautCount[i.servicio_id]   ||0)+1 })
    ...
    el('rep-contenido').innerHTML = `
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:1rem">
        <div class="card">
          ...
          ${delitosOrdenados.length ? `<div style="display:flex;flex-direction:column;gap:.5rem">
            ${delitosOrdenados.map(([tipo,n]) => {
              ...
              return `<div style="display:grid;grid-template-columns:140px 1fr 30px;align-items:center;gap:.4rem;font-size:.76rem">
                ...
                <div style="background:var(--bg-alt,#f0f0f0);border-radius:3px;height:10px;overflow:hidden"><div style="height:100%;width:${pct}%;background:var(--verde);border-radius:3px"></div></div>
                <span style="font-weight:700;text-align:right">${n}</span>
              </div>`
            }).join('')}
          </div>` : '<div style="color:var(--muted);font-size:.8rem">Sin delitos registrados</div>'}
        </div>
        <div class="card">
          <div class="sec-titulo" style="margin-bottom:.75rem">Controles por tipo</div>
          ${controles?.length ? (() => {
            const tipos=[{label:'Id. Preventivos',key:'identidad_preventivos'},{label:'Id. Investigativos',key:'identidad_investigativos'},{label:'Migratorios',key:'migratorios'},{label:'Vehiculares',key:'vehiculares'},{label:'Flagrancias',key:'flagrancias'}]
            const tots=tipos.map(t=>({label:t.label,n:(controles||[]).reduce((a,c)=>a+(c[t.key]||0),0)}))
            const maxC=Math.max(...tots.map(t=>t.n),1)
            return `<div style="display:flex;flex-direction:column;gap:.5rem">${tots.map(t=>{const pct=Math.round((t.n/maxC)*100);return `<div style="display:grid;grid-template-columns:140px 1fr 40px;align-items:center;gap:.4rem;font-size:.76rem"><span>${t.label}</span><div style="background:var(--bg-alt,#f0f0f0);border-radius:3px;height:10px;overflow:hidden"><div style="height:100%;width:${pct}%;background:#1565C0;border-radius:3px"></div></div><span style="font-weight:700;text-align:right">${t.n.toLocaleString('es-CL')}</span></div>`}).join('')}</div>`
          })() : '<div style="color:var(--muted);font-size:.8rem">Sin controles registrados</div>'}
        </div>
      </div>

      ${cuartOrdenados.length>1?`<div class="card" style="margin-bottom:1rem">
        <div class="sec-titulo" style="margin-bottom:.75rem">Servicios por cuartel</div>
        <div style="display:flex;flex-direction:column;gap:.45rem">
          ${cuartOrdenados.map(([nom,d])=>{const pct=Math.round((d.comp/Math.max(d.svcs,1))*100);const col=pct>=80?'var(--verde)':pct>=50?'var(--amarillo)':'var(--rojo)';return `<div style="display:grid;grid-template-columns:170px 1fr 100px;align-items:center;gap:.5rem;font-size:.78rem"><span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${nom}</span><div style="background:var(--bg-alt,#f0f0f0);border-radius:3px;height:10px;overflow:hidden"><div style="height:100%;width:${pct}%;background:${col};border-radius:3px"></div></div><span style="font-size:.73rem;color:var(--muted)">${d.comp}/${d.svcs} (${pct}%)</span></div>`}).join('')}
        </div>
      </div>`:''}
      ...
      <div class="card" style="padding:0;overflow:hidden">
        ...
        <div id="tabla-detalle-body" style="display:none;overflow-x:auto">
          <table style="width:100%;border-collapse:collapse;font-size:.75rem">
            ...
            <tbody>
              ${(servicios||[]).map((s,i)=>`<tr style="${i%2===0?'background:var(--tabla-datos)':''};border-bottom:1px solid var(--border)">
                ...
                <td style="padding:.32rem .7rem;text-align:center">${visitasCount[s.id]||0}</td>
                <td style="padding:.32rem .7rem;text-align:center">${incautCount[s.id]||0}</td>
                <td style="padding:.32rem .7rem;text-align:center">${intelCount[s.id]||0}</td>
              ...
              ${!totalSvcs?`<tr><td colspan="${puedeVerTodos?8:7}" style="padding:1.5rem;text-align:center;color:var(--muted)">Sin servicios en el período</td></tr>`:''}
            </tbody>
          </table>
        </div>
      </div>`
...
async function exportarCuentaDelitos() {
  ...
  try {
    ...
    let svcsQ = APP.sb.from('servicios')
      .select('id,fecha,tipo_servicio,hora_inicio,cuartel_id,cuartel:cuarteles(nombre,comuna)')
      .gte('fecha', desde).lte('fecha', hasta).eq('estado','completado')

    if (cuartelFilt) {
      svcsQ = svcsQ.eq('cuartel_id', cuartelFilt)
    } else if (!puedeVerTodos) {
      const cid = APP.cuartelActivo()?.id || APP.cuartel?.id
      if (cid) svcsQ = svcsQ.eq('cuartel_id', cid)
    }

    const { data: servicios } = await svcsQ
    if (!servicios?.length) { toast(`Sin servicios completados entre ${desde} y ${hasta}`, 'warn'); return }

    const svcIds=servicios.map(s=>s.id), svcMap={}
    ...
    const HEADERS=['UNIDAD PROCEDIMIENTO','LEY','DELITO','N° PARTE POLICIAL Y/O OFICIO EN CASO INGRESO ILEGAL','FECHA','DESTINO DEL PARTE / OFICIO (FISCALIA Y/O P.D.I)','AUTODENUNCIA / FLAGRANCIA / CONTROL O FISCALIZACION (indicar una)','LATITUD','LONGITUD','CANTIDAD','ZONA','INCAUTACION','HORA','MODO OPERANDI','PREFECTURA','SECTOR_UNIDAD_DEL_PROCEDIMIENTO','DIA','MES','AÑO','TIPO_DELITO','COMUNA','SECTOR DESTACAMENTO (F)','CLASIFICACION','SECTOR FRONTERIZO DETENCION','RANGO HORA','NOMBRES Y APELLIDOS','FEMENINO','EDAD','DOMICILIO','NACIONALIDAD','LUGAR POR DONDE EL CIUDADANO SEÑALA QUE HIZO INGRESO AL PAIS','LATITUD','LONGITUD','ORGANISMO QUIEN DETECTO EL INGRESO (CARABINEROS, ARMADA O EJERCITO)','EN CASO DE UN NNA, PARENTESCO DEL ADULTO QUE LO ACOMPAÑA','SIN DOC']

    const rows=[]
    for (const p of personas) {
      const svc=svcMap[p.servicio_id]; if(!svc) continue
      const punto=puntosMap[p.punto_id]||{}
      const f=new Date(svc.fecha+'T12:00:00')
      ...
      const modoDesc=modosMap[p.modo_operandi_id]||'—'
      rows.push([
        ...
        _tipoDelitoParaCuenta(p.tipo_delito,p.tipo_resultado),
        svc.cuartel?.comuna||'ARICA',
        svc.cuartel?.nombre?.replace(' (F)','')||'—',
        clasif, punto.sector_fronterizo||'—',
        _rangoHora(p.hora_evento||svc.hora_inicio),
        nombre, p.sexo==='femenino'?'SI':'',
...
function _leyDesdeDelito(tipoDelito, tipoResultado) {
  if (tipoDelito && LEY_POR_DELITO[tipoDelito]) return LEY_POR_DELITO[tipoDelito]
  if (tipoResultado === 'infraccion_migratoria') return 'Ley 21.325 (Migración)'
  if (tipoResultado === 'nna_irregular')         return 'Ley 21.325 (Migración)'
  return 'Código Penal'
}
