    ...
    <div class="container">
      <div class="flex-sb" style="margin-bottom:1rem">
        ...
        <div style="display:flex;gap:.5rem;flex-wrap:wrap">
          <button class="btn btn-secundario" onclick="abrirGenerarServicio()">
            ✚ Generar servicio
          </button>
          ${APP.esAdministrador() ? `
        ...
        <div class="modal-box">
          ...
          <div class="g2" style="margin-bottom:1rem">
            <div class="campo">
              ...
              <input type="date" id="nuevo-svc-fecha"/>
            </div>
            <div class="campo">
              <label>Tipo de servicio</label>
              <select id="nuevo-svc-tipo"></select>
            </div>
            <div class="campo">
              <label>Hora inicio</label>
              <input type="time" id="nuevo-svc-hini" value="08:00"/>
            </div>
            <div class="campo">
              <label>Hora término</label>
              <input type="time" id="nuevo-svc-hfin" value="20:00"/>
            </div>
            <div class="campo">
              <label>Turno</label>
            ...
            <div class="campo">
              ...
              <input type="number" id="nuevo-svc-func" min="0" value="2"/>
            </div>
            <div class="campo">
              <label>Cantidad vehículos</label>
              <input type="number" id="nuevo-svc-veh" min="0" value="1"/>
            </div>
            <div class="campo" id="nuevo-svc-cuartel-campo">
              <label>Cuartel</label>
              <select id="nuevo-svc-cuartel"></select>
            </div>
          </div>
...
function abrirGenerarServicio() {
  ...
  if (cuartelEl) {
    cuartelEl.innerHTML = ''
    ;(APP.todosCuarteles || []).forEach(c => {
      const o = document.createElement('option')
      o.value = c.id; o.textContent = c.nombre.replace(' (F)', '')
      if (c.id === APP.cuartelActivo()?.id) o.selected = true
      cuartelEl.appendChild(o)
    })
  }
...
async function confirmarNuevoServicio() {
  ...
  if (!fecha || !tipo) {
    if (res) res.innerHTML = '<span style="color:var(--rojo)">Fecha y tipo de servicio son obligatorios.</span>'
    return
  }
  if (!cuartelId) {
    if (res) res.innerHTML = '<span style="color:var(--rojo)">Selecciona un cuartel.</span>'
    return
  }
  ...
  const { error } = await APP.sb.from('servicios').insert({
    ...
    turno,
    cantidad_funcionarios: func_,
    cantidad_vehiculos:    vehs,
    estado:                'pendiente',
  })

  if (error) {
    if (res) res.innerHTML = `<span style="color:var(--rojo)">Error: ${error.message}</span>`
    if (btnCrear) { btnCrear.disabled = false; btnCrear.textContent = '✚ Crear servicio' }
    return
  }

  if (res) res.innerHTML = '<span style="color:var(--verde)">Servicio creado correctamente.</span>'
  setTimeout(() => {
    el('modal-nuevo-servicio').style.display = 'none'
    renderServicios()
  }, 1000)
}
...
function htmlFormServicio(svc, puntos, controles, visitas, yaCompletado) {
  ...
  return `
    ...
    <!-- BLOQUE JUSTIFICACION SERVICIO SIN LABOR (visible solo si no hay puntos) -->
    <div id="bloque-sin-puntos" style="display:none;margin-bottom:1rem">
      <div style="background:#FFF3CD;border:2px solid var(--amarillo);border-radius:8px;padding:.85rem 1rem">
        <div style="font-weight:700;color:#856404;font-size:.85rem;margin-bottom:.5rem">
          ⚠ Servicio sin puntos visitados — Justificación obligatoria
        </div>
        <div style="font-size:.78rem;color:#6c5700;margin-bottom:.6rem">
          Este servicio no registra puntos visitados. Antes de guardar debe indicar el motivo
          por el cual no se realizó labor de patrullaje (ej: traslado de detenidos, comisión
          especial, corte de camino, etc.).
        </div>
        <textarea id="sin-puntos-justificacion" rows="3"
          placeholder="Describa el motivo por el cual el servicio no pudo realizar labor de patrullaje (mínimo 15 caracteres)..."
...
function togglePunto(puntoId, tipo, checked) {
  ...
  // Mostrar bloque de justificacion si no hay puntos seleccionados
  const bloqueJustif = el('bloque-sin-puntos')
  if (bloqueJustif) {
    bloqueJustif.style.display = _puntosSeleccionados.length === 0 ? 'block' : 'none'
  }
}
...
async function guardarServicio() {
  ...
  // CAMBIO v2: si no hay puntos seleccionados, exigir justificacion en observaciones
  // (el campo se muestra dinamicamente en el formulario via checkSinPuntosJustificacion)
  if (_puntosSeleccionados.length === 0) {
    const justif = el('sin-puntos-justificacion')?.value?.trim()
    if (!justif || justif.length < 15) {
      toast('Sin puntos visitados: debe ingresar una justificación (campo resaltado) antes de guardar.', 'err')
      const jEl = el('sin-puntos-justificacion')
      if (jEl) {
        jEl.style.border = '2px solid var(--rojo)'
        jEl.focus()
        jEl.scrollIntoView({ behavior: 'smooth', block: 'center' })
      } else {
        // El campo aún no está visible: forzarlo visible
        const bloque = el('bloque-sin-puntos')
        if (bloque) { bloque.style.display = 'block'; bloque.scrollIntoView({ behavior: 'smooth', block: 'center' }) }
      }
      return
    }
  }
  ...
  try {
    ...
    // Si el servicio no tuvo puntos visitados, guardar la justificacion en observaciones
    ...
    await APP.sb.from('servicios').update({
      ...
      // Justificacion por servicio sin labor (campo observaciones del propio servicio)
