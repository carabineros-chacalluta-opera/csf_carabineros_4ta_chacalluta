// ============================================================
// SISTEMA CSF OPERATIVA — servicios.js
// Lista de servicios + formulario completar
// ============================================================
...
async function renderServicios() {
  ...
  el('pantalla-servicios').innerHTML = `
    <div class="container">
      ...
      <!-- Pendientes urgentes -->
      ${pendientes.filter(s => {
        const dias = Math.ceil((new Date() - new Date(s.fecha+'T12:00:00'))/864e5)
        return dias > 2
      }).length > 0 ? `
  ...
  return `
    <div class="fila-servicio fila-${s.estado} ${urgente?'fila-urgente':''}"
      ...
      <button class="btn btn-sm btn-completar"
        onclick="event.stopPropagation();abrirFormServicio('${s.id}')">
        Completar →
      </button>` : '<span class="fila-check">✓</span>'}
    </div>`
}
...
async function procesarExcel() {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const tipo = String(row[1]).trim().replace(/^\s+/,'')
  ...
  res.innerHTML = `<span style="color:var(--verde)">✅ ${importados} servicios importados · ${ignorados} ignorados</span>`
  setTimeout(() => {
...
async function abrirFormServicio(servicioId) {
  if (APP.esComisario()) return // comisario solo visualiza
  ...
  el('form-servicio-contenido').innerHTML = htmlFormServicio(svc, puntos, controlesExist, visitasExist)
}

function htmlFormServicio(svc, puntos, controles, visitas) {
  ...
  return `
    ...
    <!-- S1: Puntos visitados -->
    <div class="form-seccion">
      <div class="form-seccion-titulo">S1 — Puntos visitados en este servicio</div>
      ...
      <div class="puntos-seleccionados" id="puntos-sel-resumen">
        ${_puntosSeleccionados.length ? `✅ Seleccionados: ${_puntosSeleccionados.map(id => _puntosDelCuartel.find(p=>p.id===id)?.nombre||id).join(' · ')}` : ''}
      </div>
    </div>
    ...
    <div style="display:flex;gap:.75rem;padding-top:1rem;border-top:1px solid var(--border)">
      <button class="btn btn-primario" onclick="guardarServicio()">✓ Guardar servicio</button>
...
function toggleGrupo(tipo) {
  const el_g = el(`grupo-${tipo}`)
  if (el_g) el_g.style.display = el_g.style.display === 'none' ? 'block' : 'none'
}
...
function togglePunto(puntoId, tipo, checked) {
  ...
  const countEl = el(`count-${tipo === 'hito' ? 'hitos' : tipo}`)
...
function toggleSeccion(nombre, mostrar) {
  ...
  el(`btn-${nombre.substring(0,5)}-si`)?.classList.toggle('btn-toggle-active', mostrar)
  el(`btn-${nombre.substring(0,5)}-no`)?.classList.toggle('btn-toggle-active', !mostrar)
}
...
async function validarCodigo(codigo) {
  ...
  const { data } = await APP.sb.from('personal_cuartel')
    .select('id').eq('codigo_funcionario', codigo).eq('cuartel_id', APP.cuartel.id).eq('activo', true).single()
...
function agregarIncautacion() {
  ...
  div.innerHTML = `
    ...
    <div class="g2">
      ...
      <div class="campo"><label>Valor $CLP</label><input type="number" class="inc-clp" min="0" placeholder="0" oninput="calcUF(this,'${id}')"/></div>
...
async function calcUF(input, id) {
  const clp = parseFloat(input.value) || 0
  const ufEl = input.closest('.g2')?.querySelector('.inc-uf') || el('inc-uf-' + id)
  if (!ufEl) return
  if (clp === 0) { ufEl.value = '—'; return }
  const uf = await obtenerValorUF(hoyISO())
...
function onDelitoChange(sel, id) {
  ...
  if (sel.value === 'trafico_drogas') {
    sub.innerHTML = `<div class="g2">
      <div class="campo"><label>Sustancia</label>
        <select class="pers-sustancia">
          <option>Marihuana</option><option>Cocaína</option>
          <option>Pasta base</option><option>Heroína</option>
          <option>Sintéticas</option><option>Otra</option>
        </select>
      </div>
      <div class="campo"><label>Modalidad ocultamiento</label>
    ...
    sub.innerHTML = `<div class="g2">
      <div class="campo"><label>Subtipo contrabando</label>
        <select class="pers-sub-contrabando">
          <option value="cigarrillos">Cigarrillos</option>
          <option value="fitozoosanitario">Fitozoosanitario</option>
          <option value="dinero">Dinero en efectivo</option>
    ...
    sub.innerHTML = `<div class="campo"><label>Subtipo Ley 17.798</label>
      <select class="pers-sub-armas">
        <option value="porte_ilegal">Porte ilegal</option>
        <option value="tenencia">Tenencia ilegal</option>
        <option value="trafico">Tráfico de armas</option>
...
async function guardarServicio() {
  ...
  const btn   = qs('[onclick="guardarServicio()"]')
  if (btn) { btn.disabled = true; btn.textContent = 'Guardando...' }

  try {
    // Actualizar código responsable y estado
    const codigo = el('codigo-resp')?.value?.trim()
    await APP.sb.from('servicios').update({
      codigo_jefe_servicio: codigo || null,
      estado:               'completado',
    ...
    await APP.sb.from('visitas_puntos').delete().eq('servicio_id', svcId)
    await APP.sb.from('controles_servicio').delete().eq('servicio_id', svcId)
    await APP.sb.from('observaciones_intel').delete().eq('servicio_id', svcId)
    ...
    for (const obs of obsItems) {
      ...
      // Si relevancia alta → crear reporte pendiente
    ...
    // Guardar S4 — Incautaciones
    await APP.sb.from('incautaciones').delete().eq('servicio_id', svcId)
    const incItems = qsa('.sub-form[id^="inc-"]')
    const valorUF = await obtenerValorUF(fecha)
    ...
    // Guardar S5 — Hallazgos sin detenido
    await APP.sb.from('hallazgos_sin_detenido').delete().eq('servicio_id', svcId)
    ...
    // Guardar S6 — Personas
    await APP.sb.from('personas_registradas').delete().eq('servicio_id', svcId)
    ...
    for (const p of persItems) {
      ...
      await APP.sb.from('personas_registradas').insert({
        ...
        ley_aplicable:         p.querySelector('.pers-ley')?.value || null,
      ...
      if (esCohecho) {
        await APP.sb.from('alertas').insert({ cuartel_id: APP.cuartel.id, tipo: 'cohecho',
          detalle: `Cohecho detectado en servicio ${fecha}`, servicio_id: svcId })
      }
      if (esNNA) {
        await APP.sb.from('alertas').insert({ cuartel_id: APP.cuartel.id, tipo: 'nna',
          detalle: `NNA en situación irregular detectado - ${fecha}`, servicio_id: svcId })
      }
      if (esInterpol) {
        await APP.sb.from('alertas').insert({ cuartel_id: APP.cuartel.id, tipo: 'interpol',
