...
function onTipoEspecieChange(sel, id) {
  ...
  if (sel.value === 'droga') {
    extra.innerHTML = `
      <div class="g2" style="margin-top:.5rem;padding-top:.5rem;border-top:1px dashed var(--border)">
        <div class="campo"><label>Sustancia</label>
          <select class="pers-sustancia">
            <option value="marihuana">Marihuana</option>
            <option value="cocaina">Cocaína</option>
            <option value="pasta_base">Pasta base</option>
            <option value="heroina">Heroína</option>
            <option value="sinteticas">Sintéticas</option>
...
async function guardarServicio() {
  ...
  try {
    ...
    for (const obs of obsItems) {
      ...
      const { data: obsRec, error: obsErr } = await APP.sb.from('observaciones_intel').insert({
        servicio_id:      svcId,
      ...
      if (!obsErr && nivel === 'alto' && obsRec?.id) {
        const tipoHallazgo = obs.querySelector('.obs-tipo')?.value?.replace(/_/g,' ').toUpperCase() || 'HALLAZGO'
        await APP.sb.from('reportes_inteligencia').insert({
          ...
          estado:         'pendiente',
          titulo:         `Obs. nivel ALTO — ${tipoHallazgo} (${fecha})`,
        })
      }
    }
    ...
    for (const p of persItems) {
      ...
      const { data: persRec, error: persErr } = await APP.sb.from('personas_registradas').insert({
        servicio_id:              svcId,
        punto_id:                 puntoId || null,
      ...
      if (persErr) { console.error('Error guardando persona:', persErr.message) }
