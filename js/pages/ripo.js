...
async function generarRipo() {
  ...
  const [
    ...
    // FIX-R1: seleccionar todas las columnas incluyendo las nuevas
    APP.sb.from('entrevistas_servicio').select('*,punto:puntos_territoriales(nombre)').in('servicio_id', svcIds),
    // FIX-R2: seleccionar todas las columnas incluyendo las nuevas
...
function generarHojaFrecuencia(puntos, visitas, tipo, anio) {
  ...
  ;(visitas||[]).forEach(v => {
    ...
    const semana = semanaISO(v.fecha)
    if (!semanasPorPunto[v.punto_id]) semanasPorPunto[v.punto_id] = new Set()
    semanasPorPunto[v.punto_id].add(semana)
  })
  const FVC_ANUAL = { 'diario':365,'2x_semana':104,'semanal':52,'quincenal':26,'mensual':12,'bimestral':6,'trimestral':4,'semestral':2 }
  const semHeaders = Array.from({length:52},(_,i)=>`S${i+1}`)
  const headerBase = ['PREFECTURA','CUARTEL (F)','NUMERO PUNTO','NOMBRE PUNTO','PAÍS LIMITROFE','TIEMPO VERIFICACION (min)','TIEMPO TRASLADO (min)','TOTAL MIN','TOTAL HORAS','FRECUENCIA ANUAL','CANTIDAD VERIFICACION ANUAL',...semHeaders]
  ...
  puntosTipo.forEach(p => {
    ...
    rows.push([p.cuartel?.prefectura||'ARICA N° 1',p.cuartel?.nombre||'',p.nombre,p.nombre_completo||p.nombre,p.pais_limitrofe||'',tiempoVerif,tiempoTrasl,totalMin,totalHoras,CSF_CONFIG.FVC_LABELS[p.fvc_base]||p.fvc_base,fvcAnual,...semanas])
  })
  return XLSX.utils.aoa_to_sheet(rows)
}
