...
async function renderCSF() {
  ...
  el('pantalla-csf').innerHTML = `
    <div class="container">
      ${puedeVerTodos ? htmlBannerCuartel() : ''}
      <div class="tabs-bar">
...
async function generarBorradorCSF() {
  ...
  if (!cuartelActivo?.id) { zona.innerHTML = htmlSinCuartelSeleccionado('el cuartel para generar la CSF'); return }
...
async function renderSeguimiento() {
  ...
  if (!cuartelId) { zona.innerHTML = htmlSinCuartelSeleccionado('el seguimiento de CSF activa'); return }
              ...
              <tr style="${i%2===0?'background:#fafafa':''};border-bottom:1px solid var(--border)">
                ...
                <td style="padding:.35rem .6rem;text-align:center">
                  <div style="display:flex;gap:.35rem;justify-content:center;flex-wrap:wrap">
                    ...
                    ${c.estado !== 'borrador' ? `<button class="btn btn-sm" style="background:#e8f0fe;color:#0055d4;border:1px solid #90CAF9" onclick="verCumplimientoCSF('${c.id}')">📊 Cumplimiento</button>` : ''}
  ...
  modal.innerHTML = `
    <div style="background:#fff;border-radius:12px;width:min(98vw,900px);max-height:94vh;display:flex;flex-direction:column">
      ...
      <div style="flex:1;overflow-y:auto;padding:1rem" id="csf-print-area-modal">
        ${await _htmlCSFCompleta(csf, visitas||[], puntosFvc||[])}
      </div>
    </div>`
  document.body.appendChild(modal)
}
...
async function imprimirCSFHistorial(csfId) {
  ...
  const contenido = await _htmlCSFCompleta(csf, visitas||[], puntosFvc||[])
...
async function _htmlCSFCompleta(csf, visitas, puntosFvc) {
  ...
  const bloqueValidador = csf.firma_nombre ? `
    <div style="padding:1rem;text-align:center">
      ${(csf.firma_imagen_url || csf.firma_b64)
        ? `<img src="${csf.firma_imagen_url || csf.firma_b64}" style="height:60px;max-width:200px;object-fit:contain;display:block;margin:0 auto .4rem"/>`
        : '<div style="height:60px"></div>'}
      <div style="border-top:2px solid #000;padding-top:.35rem;margin-top:.25rem">
...
function modalOpcionesImpresion(csfId, numero) {
  ...
  modal.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:9999;display:flex;align-items:center;justify-content:center;padding:1rem'
  modal.innerHTML = `
    <div style="background:var(--surface,#fff);border-radius:12px;width:min(96vw,460px);overflow:hidden">
      <div style="padding:.9rem 1.1rem;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center">
        <span style="font-weight:700;font-size:.92rem">Imprimir — ${numero}</span>
        <button onclick="el('modal-opc-imp').remove()" style="border:none;background:none;font-size:1.2rem;cursor:pointer">X</button>
      </div>
      <div style="padding:1rem;display:flex;flex-direction:column;gap:.55rem">
        <button class="btn btn-secundario" style="text-align:left;padding:.7rem .9rem;height:auto"
          onclick="el('modal-opc-imp').remove();imprimirCSF('${csfId}','completa')">
          <div style="font-weight:600;font-size:.84rem">Con firma digital</div>
          <div style="font-size:.72rem;color:var(--muted);margin-top:.15rem">Incluye imagen de firma del comisario y del SPF</div>
        </button>
        <button class="btn btn-secundario" style="text-align:left;padding:.7rem .9rem;height:auto"
...
async function modalImprimirTodas() {
  ...
  modal.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:9999;display:flex;align-items:center;justify-content:center;padding:1rem'
  modal.innerHTML = `
    <div style="background:var(--surface,#fff);border-radius:12px;width:min(96vw,500px);overflow:hidden">
      <div style="padding:.9rem 1.1rem;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center">
        <span style="font-weight:700;font-size:.92rem">Imprimir todas las CSF</span>
      ...
      <div style="padding:1rem">
        ...
        <div class="campo" style="margin-bottom:.75rem">
          <label>Estado</label>
          <select id="imp-estado" style="width:100%">
            <option value="publicada">Publicadas (vigentes)</option>
...
async function ejecutarImpresionTodas() {
  const cuartelFilt = el('imp-cuartel')?.value || null
...
// ============================================================
// SEGUIMIENTO HISTÓRICO — Cumplimiento de CSF pasadas
// ============================================================

// Helper: construye string de fecha local (evita desfase UTC)
const _isoFecha = d =>
  `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`

// Devuelve los períodos con conteo real de visitas requeridas vs ejecutadas
function _periodosCumplimiento(puntoId, visitasEjec, iniVig, finVig, fvc) {
  const visitasPunto = visitasEjec
    .filter(v => v.punto_id === puntoId && v.fecha >= iniVig && v.fecha <= finVig)
  const fechasVisitadas = visitasPunto.map(v => v.fecha)

  const ini = new Date(iniVig + 'T12:00:00')
  const fin = new Date(finVig + 'T12:00:00')
  const periodos = []

  const _semanaLabel = (d) => {
    ...
    return `Sem.${semanaISO(_isoFecha(d))} (${fmt(lunes)}-${fmt(domingo)})`
  }

  if (fvc === 'diario') {
    // Agrupar por semana para no tener 30 filas
    const weekData = {}
    const cur = new Date(ini)
    while (cur <= fin) {
      const iso  = _isoFecha(cur)
      const nSem = semanaISO(iso)
      if (!weekData[nSem]) weekData[nSem] = { label: _semanaLabel(cur), requeridas: 0, dias: [] }
      weekData[nSem].requeridas++
      weekData[nSem].dias.push(iso)
      cur.setDate(cur.getDate() + 1)
    }
    for (const w of Object.values(weekData)) {
      const fv = fechasVisitadas.filter(f => w.dias.includes(f))
      periodos.push({ label: w.label, requeridas: w.requeridas, ejecutadas: fv.length, fechasVisita: fv })
    }
    return periodos
  }

  if (fvc === 'semanal' || fvc === '2x_semana') {
    const reqPorSemana = fvc === '2x_semana' ? 2 : 1
    const semanasVistas = new Set()
    const cur = new Date(ini)
    while (cur <= fin) {
      const nSem = semanaISO(_isoFecha(cur))
      if (!semanasVistas.has(nSem)) {
        semanasVistas.add(nSem)
        const label = _semanaLabel(cur)
        const dow    = cur.getDay() || 7
        const lunes  = new Date(cur); lunes.setDate(cur.getDate() - (dow - 1))
        const diasSem = []
        const c2 = new Date(lunes)
        for (let i = 0; i < 7; i++) {
          const iso = _isoFecha(c2)
          if (iso >= iniVig && iso <= finVig) diasSem.push(iso)
          c2.setDate(c2.getDate() + 1)
        }
        const fv = fechasVisitadas.filter(f => diasSem.includes(f))
        periodos.push({ label, requeridas: reqPorSemana, ejecutadas: fv.length, fechasVisita: fv })
      }
      cur.setDate(cur.getDate() + 7)
    }
    return periodos
  ...
  if (fvc === 'quincenal') {
    ...
    const m  = ini.getMonth()
    const ult = new Date(y, m + 1, 0).getDate()
    const mes = MESES_ES[m].substring(0, 3)
    const q1i = `${y}-${String(m+1).padStart(2,'0')}-01`
    const q1f = `${y}-${String(m+1).padStart(2,'0')}-15`
    const q2i = `${y}-${String(m+1).padStart(2,'0')}-16`
    const q2f = `${y}-${String(m+1).padStart(2,'0')}-${String(ult).padStart(2,'0')}`
    const addQ = (label, desde, hasta) => {
      const fv = fechasVisitadas.filter(f => f >= desde && f <= hasta)
      periodos.push({ label, requeridas: 1, ejecutadas: fv.length, fechasVisita: fv })
    }
    addQ(`Q1 ${mes} (01-15)`,   q1i, q1f)
    addQ(`Q2 ${mes} (16-${ult})`, q2i, q2f)
    return periodos
  }

  // mensual / bimestral / trimestral / semestral → 1 período
  const fv = fechasVisitadas.slice()
  periodos.push({
    label: `${MESES_ES[ini.getMonth()]} ${ini.getFullYear()}`,
    requeridas: 1, ejecutadas: fv.length, fechasVisita: fv
  })
  return periodos
}

function _kpiCump(label, valor, color) {
  return `<div style="background:var(--surface-2);border-radius:8px;padding:.6rem .85rem;flex:1;min-width:90px">
    <div style="font-size:1.25rem;font-weight:700;color:${color}">${valor}</div>
    <div style="font-size:.68rem;color:var(--muted);margin-top:.1rem">${label}</div>
  </div>`
}

function toggleDetallePunto(safe) {
  const det = document.getElementById('det_' + safe)
  if (!det) return
  const abre = det.style.display === 'none'
  det.style.display = abre ? 'block' : 'none'
  const ico = document.getElementById('ico_' + safe)
  if (ico) ico.textContent = abre ? '▲' : '▼'
}

async function verCumplimientoCSF(csfId) {
  // Crear / reutilizar modal
  let modal = document.getElementById('modal-cumplimiento-csf')
  if (!modal) {
    modal = document.createElement('div')
    modal.id = 'modal-cumplimiento-csf'
    modal.style.cssText = 'position:fixed;inset:0;z-index:9998;background:rgba(0,0,0,.55);display:flex;align-items:flex-start;justify-content:center;overflow-y:auto;padding:1.5rem 1rem'
    document.body.appendChild(modal)
  }
  modal.style.display = 'flex'
  modal.innerHTML = `<div style="background:#fff;border-radius:12px;padding:2rem;max-width:920px;width:100%;margin:auto"><div class="cargando">Calculando cumplimiento...</div></div>`

  try {
    // ── 1. Datos de la CSF ──
    const { data: csf, error: e1 } = await APP.sb.from('csf_mensual')
      .select('*,cuartel:cuarteles(nombre)').eq('id', csfId).single()
    if (e1 || !csf) throw new Error('No se encontró la CSF')

    const iniVig    = csf.fecha_vigencia_inicio
    const finVig    = csf.fecha_vigencia_fin
    const cuartelId = csf.cuartel_id

    // ── 2. Visitas planificadas en la CSF ──
    const { data: visitasPlan } = await APP.sb.from('csf_visitas_ordenadas')
      .select('*,punto:puntos_territoriales(id,nombre,nombre_completo,tipo,fvc_base)')
      .eq('csf_id', csfId).order('fecha_ordenada')

    // ── 3. Servicios y visitas ejecutadas en el período ──
    const { data: svcsVig } = await APP.sb.from('servicios')
      .select('id').eq('cuartel_id', cuartelId)
      .gte('fecha', iniVig).lte('fecha', finVig)
    const svcIds = (svcsVig||[]).map(s => s.id)

    let visitasEjec = []
    if (svcIds.length) {
      const { data: ve } = await APP.sb.from('visitas_puntos')
        .select('punto_id,fecha').in('servicio_id', svcIds)
      visitasEjec = ve || []
    }

    // ── 4. Calcular cumplimiento por punto ──
    const puntosIds = [...new Set((visitasPlan||[]).map(v => v.punto?.id).filter(Boolean))]
    const puntosSeg = puntosIds.map(pid => {
      const pInfo = visitasPlan.find(v => v.punto?.id === pid)?.punto
      if (!pInfo) return null
      const fvc      = pInfo.fvc_base || 'mensual'
      const periodos = _periodosCumplimiento(pid, visitasEjec, iniVig, finVig, fvc)
      const ejecutadas  = visitasEjec.filter(v => v.punto_id === pid && v.fecha >= iniVig && v.fecha <= finVig).length
      const requeridas  = periodos.reduce((a,p) => a + p.requeridas, 0)
      const pCumplidos  = periodos.filter(p => p.ejecutadas >= p.requeridas).length
      const pTotal      = periodos.length
      const pct         = pTotal > 0 ? Math.round(pCumplidos / pTotal * 100) : 0
      return { ...pInfo, fvc, periodos, ejecutadas, requeridas, pCumplidos, pTotal, pct }
    }).filter(Boolean).sort((a,b) => a.pct - b.pct)

    const totPeriodos  = puntosSeg.reduce((a,p) => a + p.pTotal, 0)
    const totCumplidos = puntosSeg.reduce((a,p) => a + p.pCumplidos, 0)
    const pctGlobal    = totPeriodos > 0 ? Math.round(totCumplidos / totPeriodos * 100) : 0
    const colGlobal    = pctGlobal >= 90 ? 'var(--verde)' : pctGlobal >= 70 ? 'var(--amarillo)' : 'var(--rojo)'

    // ── 5. Render ──
    modal.innerHTML = `
    <div style="background:#fff;border-radius:12px;overflow:hidden;max-width:920px;width:100%;margin:auto">

      <!-- Cabecera -->
      <div style="background:var(--verde);color:#fff;padding:1.25rem 1.5rem;display:flex;justify-content:space-between;align-items:start;gap:1rem">
        <div>
          <div style="font-size:1rem;font-weight:700">📊 Cumplimiento CSF</div>
          <div style="font-size:.88rem;opacity:.92;margin-top:.2rem">${csf.numero} — ${csf.cuartel?.nombre?.replace(' (F)','')||''}</div>
          <div style="font-size:.75rem;opacity:.8;margin-top:.15rem">Vigencia: ${formatFechaCorta(iniVig)} → ${formatFechaCorta(finVig)}</div>
        </div>
        <div style="text-align:right;flex-shrink:0">
          <div style="font-size:2.8rem;font-weight:700;line-height:1;color:#fff">${pctGlobal}%</div>
          <div style="font-size:.7rem;opacity:.85">${totCumplidos}/${totPeriodos} períodos cumplidos</div>
          <button onclick="document.getElementById('modal-cumplimiento-csf').style.display='none'"
            style="margin-top:.6rem;background:rgba(255,255,255,.2);border:1px solid rgba(255,255,255,.5);
                   color:#fff;border-radius:6px;padding:.3rem .75rem;cursor:pointer;font-size:.8rem">
            ✕ Cerrar
          </button>
        </div>
      </div>
      ...
      <!-- KPIs resumen -->
      <div style="display:flex;gap:.6rem;padding:1rem 1.5rem;flex-wrap:wrap;background:var(--surface-2);border-bottom:1px solid var(--border)">
        ${_kpiCump('Cumplimiento global', pctGlobal+'%', colGlobal)}
        ${_kpiCump('Períodos cumplidos', totCumplidos+'/'+totPeriodos, 'var(--text)')}
        ${_kpiCump('Puntos 100%', puntosSeg.filter(p=>p.pct===100).length+'/'+puntosSeg.length, 'var(--verde)')}
        ${_kpiCump('Total visitas ejecutadas', puntosSeg.reduce((a,p)=>a+p.ejecutadas,0), 'var(--text)')}
        ${_kpiCump('Visitas requeridas', puntosSeg.reduce((a,p)=>a+p.requeridas,0), 'var(--muted)')}
      </div>

      <!-- Detalle por punto -->
      <div style="padding:1rem 1.5rem;max-height:65vh;overflow-y:auto">
        <div style="font-size:.78rem;font-weight:700;color:var(--muted);margin-bottom:.5rem;padding-left:.2rem">
          DETALLE POR PUNTO — haz clic en una fila para ver el desglose por períodos
        </div>

        ${puntosSeg.length === 0 ? `<div class="card" style="text-align:center;padding:2rem;color:var(--muted)">Sin puntos en esta CSF</div>` : ''}

        ${puntosSeg.map((p, i) => {
          const col   = p.pct >= 90 ? '#1A843F' : p.pct >= 50 ? '#9A7D0A' : '#C0392B'
          const colBg = p.pct >= 90 ? '#e8f5ea'  : p.pct >= 50 ? '#FFF8E1'  : '#FDECEA'
          const safe  = p.id.replace(/-/g,'_')
          const barW  = Math.min(p.pct, 100)
          return `
          <div style="border:1px solid var(--border);border-radius:8px;margin-bottom:.5rem;overflow:hidden">

            <!-- Fila resumen — clickable -->
            <div onclick="toggleDetallePunto('${safe}')"
                 style="display:grid;grid-template-columns:1fr 65px 115px 55px 55px 75px 28px;
                        align-items:center;gap:.4rem;padding:.55rem .85rem;cursor:pointer;
                        background:${i%2===0?'#fafafa':'#fff'};user-select:none">
              <div>
                <span style="font-weight:600;font-size:.8rem">${p.nombre}</span>
                ${p.nombre_completo && p.nombre_completo !== p.nombre
                  ? `<div style="font-size:.65rem;color:var(--muted)">${p.nombre_completo}</div>` : ''}
              </div>
              <div><span class="badge badge-tipo-${p.tipo}" style="font-size:.62rem">${p.tipo.toUpperCase()}</span></div>
              <div style="font-size:.7rem;color:var(--muted)">${CSF_CONFIG.FVC_LABELS[p.fvc]||p.fvc}</div>
              <div style="text-align:center;font-size:.75rem;color:var(--muted)" title="Visitas requeridas">${p.requeridas}</div>
              <div style="text-align:center;font-size:.8rem;font-weight:700;color:${p.ejecutadas>=p.requeridas?'#1A843F':'#C0392B'}"
                   title="Visitas ejecutadas">${p.ejecutadas}</div>
              <div style="text-align:center">
                <div style="background:#e8e8e8;border-radius:4px;height:8px;overflow:hidden;margin-bottom:2px">
                  <div style="height:100%;width:${barW}%;background:${col};border-radius:4px"></div>
                </div>
                <span style="background:${colBg};color:${col};font-size:.68rem;font-weight:700;
                             padding:1px 5px;border-radius:3px">${p.pct}%</span>
              </div>
              <div style="text-align:center;color:var(--muted);font-size:.8rem" id="ico_${safe}">▼</div>
            </div>

            <!-- Detalle de períodos — oculto por defecto -->
            <div id="det_${safe}" style="display:none;border-top:1px solid var(--border);background:#f9fbf9;padding:.65rem .85rem">
              <table style="width:100%;border-collapse:collapse;font-size:.72rem">
                <thead>
                  <tr style="background:var(--encabezado)">
                    <th style="padding:.28rem .5rem;text-align:left">Período</th>
                    <th style="padding:.28rem .5rem;text-align:center;width:80px">Requeridas</th>
                    <th style="padding:.28rem .5rem;text-align:center;width:80px">Ejecutadas</th>
                    <th style="padding:.28rem .5rem;text-align:center;width:60px">Estado</th>
                    <th style="padding:.28rem .5rem;text-align:left">Fechas de visita</th>
                  </tr>
                </thead>
                <tbody>
                  ${p.periodos.map((per, j) => {
                    const ok      = per.ejecutadas >= per.requeridas
                    const parcial = !ok && per.ejecutadas > 0
                    const icono   = ok ? '✅' : parcial ? '⚠️' : '❌'
                    const colorEj = ok ? '#1A843F' : parcial ? '#9A7D0A' : '#C0392B'
                    return `
                    <tr style="${j%2===0?'background:#fff':'background:#f5f8f5'}">
                      <td style="padding:.25rem .5rem;font-weight:500">${per.label}</td>
                      <td style="padding:.25rem .5rem;text-align:center;color:var(--muted)">${per.requeridas}</td>
                      <td style="padding:.25rem .5rem;text-align:center;font-weight:700;color:${colorEj}">${per.ejecutadas}</td>
                      <td style="padding:.25rem .5rem;text-align:center">${icono}</td>
                      <td style="padding:.25rem .5rem;color:var(--muted);font-size:.68rem">
                        ${per.fechasVisita.map(f => formatFechaCorta(f)).join(' · ') || '—'}
                      </td>
                    </tr>`
                  }).join('')}
                  <tr style="background:var(--tabla-datos);font-weight:700">
                    <td style="padding:.25rem .5rem">TOTAL</td>
                    <td style="padding:.25rem .5rem;text-align:center">${p.requeridas}</td>
                    <td style="padding:.25rem .5rem;text-align:center;color:${p.ejecutadas>=p.requeridas?'#1A843F':'#C0392B'}">${p.ejecutadas}</td>
                    <td style="padding:.25rem .5rem;text-align:center">
                      <span style="background:${colBg};color:${col};padding:1px 5px;border-radius:3px">${p.pct}%</span>
                    </td>
                    <td style="padding:.25rem .5rem;color:var(--muted);font-size:.68rem">
                      ${p.pCumplidos}/${p.pTotal} períodos con cumplimiento completo
                    </td>
                  </tr>
                </tbody>
              </table>
    ...
    modal.innerHTML = `
      <div style="background:#fff;border-radius:12px;padding:2rem;max-width:500px;width:100%;margin:auto">
        <div style="color:var(--rojo);font-weight:600">Error al calcular cumplimiento</div>
        <div style="font-size:.8rem;color:var(--muted);margin-top:.35rem">${e.message}</div>
        <button onclick="document.getElementById('modal-cumplimiento-csf').style.display='none'"
          class="btn btn-ghost" style="margin-top:1rem">Cerrar</button>
      </div>`
  }
}
