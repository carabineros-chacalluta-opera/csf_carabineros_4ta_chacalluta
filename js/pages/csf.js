...
async function enviarCSFRevision() {
  ...
  const { data: existente } = await APP.sb.from('csf_mensual')
...
async function renderHistorial() {
  ...
  // Validador ve todos los cuarteles; el resto solo el suyo
  let q = APP.sb.from('csf_mensual')
    .select('*,cuartel:cuarteles(nombre)')
    .order('created_at', { ascending: false }).limit(30)
  if (!esValidador && cuartelId) q = q.eq('cuartel_id', cuartelId)
...
function _htmlTarjetaRevision(csf, esValidador) {
  ...
  return `
  <div class="card" style="border-left:4px solid var(--amarillo)">
    ...
    <div style="background:#fff;border-radius:12px;width:min(96vw,780px);max-height:88vh;display:flex;flex-direction:column">
      ...
      <div style="flex:1;overflow-y:auto;padding:.5rem">
        <table style="width:100%;border-collapse:collapse;font-size:.76rem">
          ...
          <tbody>
            ${(visitas||[]).map((v,i) => `
            <tr id="fila-v-${v.id}" style="${i%2===0?'background:#fafafa':''};border-bottom:1px solid #eee">
              <td style="padding:.3rem .6rem;text-align:center;font-weight:700">${String(v.numero_visita).padStart(2,'0')}</td>
              <td style="padding:.3rem .6rem;font-weight:500">${v.punto?.nombre||'—'}</td>
              ...
              <td style="padding:.3rem .6rem">${formatFecha(v.fecha_ordenada)}</td>
              <td id="ini-${v.id}" style="padding:.3rem .6rem;font-weight:500">${v.hora_inicio}</td>
              <td id="fin-${v.id}" style="padding:.3rem .6rem;font-weight:500">${v.hora_termino}</td>
              ${esValidador ? `
              <td style="padding:.3rem .6rem;text-align:center">
                <button class="btn btn-sm btn-secundario" onclick="editarHorarioVisita('${v.id}','${v.punto?.tipo||'pnh'}')"
...
async function editarHorarioVisita(visitaId, tipoPunto) {
  const filaEl  = el(`fila-v-${visitaId}`)
  const iniEl   = el(`ini-${visitaId}`)
  const finEl   = el(`fin-${visitaId}`)
  if (!filaEl || !iniEl || !finEl) return
  ...
  div.style.cssText = 'display:flex;align-items:center;gap:.35rem'
  div.innerHTML = `
    <input type="time" id="edit-ini-${visitaId}" value="${hIniActual}"
      style="width:85px;font-size:.75rem;padding:.2rem .35rem;border:1px solid var(--border);border-radius:4px"
      ${esHito ? 'min="06:00" max="20:00"' : ''}/>
    <span style="font-size:.7rem">–</span>
    <input type="time" id="edit-fin-${visitaId}" value="${hFinActual}"
      style="width:85px;font-size:.75rem;padding:.2rem .35rem;border:1px solid var(--border);border-radius:4px"/>
    <button onclick="guardarHorarioVisita('${visitaId}')" class="btn btn-sm btn-primario" style="padding:.2rem .5rem">✓</button>
    <button onclick="cancelarEdicionHorario('${visitaId}','${hIniActual}','${hFinActual}')" class="btn btn-sm btn-ghost" style="padding:.2rem .4rem">✕</button>`

  iniEl.innerHTML = ''
  finEl.innerHTML = ''
  iniEl.colSpan = 2
  iniEl.appendChild(div)
  if (esHito) {
    const nota = document.createElement('div')
    nota.style.cssText = 'font-size:.65rem;color:#856404;margin-top:.2rem'
    nota.textContent = 'Hito: rango diurno recomendado (06:00–20:00)'
    div.appendChild(nota)
  }
}

async function guardarHorarioVisita(visitaId) {
  const hIni = el(`edit-ini-${visitaId}`)?.value
  const hFin = el(`edit-fin-${visitaId}`)?.value
  if (!hIni || !hFin) return
  ...
  const iniEl = el(`ini-${visitaId}`)
  const finEl = el(`fin-${visitaId}`)
  if (iniEl) { iniEl.innerHTML = hIni; iniEl.colSpan = 1 }
  if (finEl) finEl.innerHTML = hFin
  toast('Horario actualizado', 'ok')
}

function cancelarEdicionHorario(visitaId, hIni, hFin) {
  const iniEl = el(`ini-${visitaId}`)
  const finEl = el(`fin-${visitaId}`)
  if (iniEl) { iniEl.innerHTML = hIni; iniEl.colSpan = 1 }
  if (finEl) finEl.innerHTML = hFin
...
async function verCSFCompleta(csfId) {
  ...
  const { data: puntosFvc } = await APP.sb.from('csf_puntos_fvc')
    .select('*,punto:puntos_territoriales(nombre,tipo)').eq('csf_id', csfId)
  ...
  modal.innerHTML = `
    <div style="background:#fff;border-radius:12px;width:min(98vw,860px);max-height:92vh;display:flex;flex-direction:column">
      <div style="padding:.85rem 1.1rem;border-bottom:1px solid #e5e7eb;display:flex;justify-content:space-between;align-items:center">
        <span style="font-weight:700">${csf?.numero || 'CSF'} — ${csf?.cuartel?.nombre?.replace(' (F)','')||''}</span>
        <div style="display:flex;gap:.5rem">
          <button onclick="window.print()" class="btn btn-sm btn-secundario">↓ Imprimir</button>
...
function _htmlCSFCompleta(csf, visitas, puntosFvc) {
  if (!csf) return '<div style="color:var(--rojo)">Error al cargar CSF</div>'
  const firma = csf.firma_nombre ? `
    <div style="text-align:center;padding:.75rem 1rem">
      ${csf.firma_imagen_url ? `<img src="${csf.firma_imagen_url}" style="height:60px;margin-bottom:.35rem;display:block;margin-left:auto;margin-right:auto"/>` : '<div style="height:60px"></div>'}
      <div style="font-weight:700;font-size:.8rem">${csf.firma_nombre}</div>
      <div style="font-size:.72rem">${csf.firma_grado || ''}</div>
      <div style="font-size:.72rem">${csf.firma_cargo || ''}</div>
    </div>` : `<div style="padding:.75rem 1rem;text-align:center;color:var(--muted);font-size:.75rem">Pendiente de validación</div>`
  return `
    <div style="font-family:Arial,sans-serif;font-size:.78rem">
      <div style="background:#04742C;color:#fff;padding:.5rem .85rem;font-weight:700">CARTA DE SITUACIÓN FRONTERIZA — ${csf.numero}</div>
      <div style="display:grid;grid-template-columns:1fr 1fr;background:#CCE3D3;font-size:.7rem;padding:.3rem .6rem;gap:.5rem">
        <div><strong>Sector:</strong> ${csf.cuartel?.nombre||'—'}</div>
        <div><strong>Vigencia:</strong> ${formatFechaCorta(csf.fecha_vigencia_inicio)} → ${formatFechaCorta(csf.fecha_vigencia_fin)}</div>
        <div><strong>Clasificación:</strong> ${csf.clasificacion}</div>
        <div><strong>Estado:</strong> ${(csf.estado||'').toUpperCase().replace('_',' ')}</div>
      </div>
      <div style="background:#1A843F;color:#fff;padding:.3rem .6rem;font-weight:700;margin-top:.5rem">LUGARES A PATRULLAR</div>
      <table style="width:100%;border-collapse:collapse;font-size:.7rem">
        <thead><tr style="background:#f0f0f2">
          <th style="padding:.3rem .5rem;text-align:center">N°</th>
          <th style="padding:.3rem .5rem">Punto</th>
          <th style="padding:.3rem .5rem;text-align:center">Tipo</th>
          <th style="padding:.3rem .5rem">Fecha</th>
          <th style="padding:.3rem .5rem">Horario</th>
        </tr></thead>
        <tbody>
          ${visitas.map((v,i) => `
          <tr style="${i%2===0?'background:#E8F5EC':''}">
            <td style="padding:.25rem .5rem;text-align:center;font-weight:700">${String(v.numero_visita).padStart(2,'0')}</td>
            <td style="padding:.25rem .5rem">${v.punto?.nombre||'—'}</td>
            <td style="padding:.25rem .5rem;text-align:center;font-size:.63rem">${(v.punto?.tipo||'').toUpperCase()}</td>
            <td style="padding:.25rem .5rem">${formatFecha(v.fecha_ordenada)}</td>
            <td style="padding:.25rem .5rem">${v.hora_inicio} – ${v.hora_termino}</td>
          </tr>`).join('')}
        </tbody>
      </table>
      <div style="background:#1A843F;color:#fff;padding:.3rem .6rem;font-weight:700;margin-top:.5rem">V. FIRMAS Y VALIDACIÓN</div>
      <div style="display:grid;grid-template-columns:1fr 1fr;border:1px solid #ddd">
        <div style="border-right:1px solid #ddd;padding:.75rem;text-align:center">
          <div style="height:50px"></div>
          <div style="font-size:.72rem;font-weight:700;border-top:1px solid #000;padding-top:.2rem">Comisario · Elaborador</div>
        </div>
        <div>${firma}</div>
      </div>
    </div>`
...
async function iniciarGeneracionMasiva() {
  ...
  for (let i = 0; i < cuarteles.length; i++) {
    ...
    try {
      // Verificar si ya existe CSF para este cuartel/mes
      const { data: existente } = await APP.sb.from('csf_mensual')
