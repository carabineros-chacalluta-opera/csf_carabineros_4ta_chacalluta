...
// ── CRITICIDAD: funcion compartida por generarBorradorCSF y _generarCSFParaCuartel
// Conforme al documento oficial SPF "Niveles de Criticidad"
// Fuentes:
//   personasPunto  — S6 detenciones e infracciones migratorias del punto
//   incautPunto    — S4 incautaciones CON detenido del punto
//   hallazgosPunto — S5 hallazgos SIN detenido del punto (vehiculos, dinero)
function _calcularNivelExcel(personasPunto, incautPunto, hallazgosPunto) {
  ...
  // PERSONAS (escala por cantidad): trafico migrantes + trata + ingresos + egresos
  const nTraficoMig  = nivelDesdeDelito('personas', pp.filter(pr => pr.tipo_delito === 'trafico_migrantes').length)
  const nTrata       = nivelDesdeDelito('personas', pp.filter(pr => pr.tipo_delito === 'trata_personas').length)
  const nIngresoAdulto = nivelDesdeDelito('personas', pp.filter(pr => (pr.situacion_migratoria === 'irregular' || pr.situacion_migratoria === 'ingreso_irregular') && pr.grupo_etario === 'adulto').length)
  const nIngresoNNA    = nivelDesdeDelito('personas', pp.filter(pr => (pr.situacion_migratoria === 'irregular' || pr.situacion_migratoria === 'ingreso_irregular') && pr.grupo_etario === 'nna').length)
  const nEgresoAdulto  = nivelDesdeDelito('personas', pp.filter(pr => pr.situacion_migratoria === 'permanencia_irregular' && pr.grupo_etario === 'adulto').length)
  const nEgresoNNA     = nivelDesdeDelito('personas', pp.filter(pr => pr.situacion_migratoria === 'permanencia_irregular' && pr.grupo_etario === 'nna').length)
  const nNNAIrregular  = nivelDesdeDelito('personas', pp.filter(pr => pr.tipo_resultado === 'nna_irregular').length)

  // CASOS (escala 1/2/3/4/5+): delitos por numero de procedimientos
  const nDrogas        = nivelDesdeDelito('casos', pp.filter(pr => pr.tipo_delito === 'trafico_drogas').length)
  const nArmas         = nivelDesdeDelito('casos', pp.filter(pr => pr.tipo_delito === 'ley_17798_armas').length)
  const nAbigeato      = nivelDesdeDelito('casos', pp.filter(pr => pr.tipo_delito === 'abigeato').length)
  const nReceptacion   = nivelDesdeDelito('casos', pp.filter(pr => pr.tipo_delito === 'receptacion').length)
  const nFalsif        = nivelDesdeDelito('casos', pp.filter(pr => pr.tipo_delito === 'falsificacion_documentos').length)
  const nCohecho       = nivelDesdeDelito('casos', pp.filter(pr => pr.tipo_delito === 'cohecho').length)

  // Contrabando mercancias: detenciones S6 + incautaciones S4 con detenido
  const nContrabando   = nivelDesdeDelito('casos', pp.filter(pr => pr.tipo_delito === 'contrabando').length +
  ...
  // Vehiculos: detenciones S6 (receptacion) + S5 hallazgos sin detenido
  const nVehiculos     = nivelDesdeDelito('casos', pp.filter(pr => pr.tipo_delito === 'receptacion').length +
    hp.filter(h => ['vehiculo_encargo','vehiculo_cot','maquinaria'].includes(h.tipo_bien)).length)

  // Dinero sin detenido (S5) — contrabando / lavado
  const nDineroS5      = nivelDesdeDelito('casos', hp.filter(h => h.tipo_bien === 'dinero').length)

  return Math.max(
    nTraficoMig, nTrata,
    nIngresoAdulto, nIngresoNNA,
    nEgresoAdulto, nEgresoNNA, nNNAIrregular,
    nDrogas, nArmas, nAbigeato,
    nReceptacion, nFalsif, nCohecho,
    nContrabando, nVehiculos, nDineroS5,
    1
  )
}
...
async function generarBorradorCSF() {
  ...
  let visitasRef=[], personasRef=[], incautRef=[], obsRef=[], visitasAnual=[], hallazgosRef=[]
  ...
  const promesas = [
    ...
    svcIdsAnual.length
      ...
      : Promise.resolve({ data: [] }),
    // S5: hallazgos sin detenido (vehiculos, dinero — fuente independiente)
    svcIds.length
  ...
  const puntosProcesados = (puntos||[]).map(p => {
    ...
    const hallazgosPunto  = (hallazgosRef||[]).filter(h => h.punto_id === p.id)

    // Criticidad — conforme a documento oficial SPF "Niveles de Criticidad"
    const nivelExcel = _calcularNivelExcel(personasPunto, incautPunto, hallazgosPunto)
...
async function _generarCSFParaCuartel(cuartelId, refMes, refAnio, vigMes, vigAnio, clasif) {
  ...
  const puntosProcesados = (puntos||[]).map(p => {
    ...
    const nivelFinal  = _calcularNivelExcel(personasPunto, incautPunto, hallazgosPunto)
