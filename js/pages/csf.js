// ============================================================
// SISTEMA CSF OPERATIVA — csf.js
// Generador de CSF + seguimiento + exportación
// ============================================================
...
function tabBtn(tab, label) {
  ...
  return `<button class="tab-btn ${act?'tab-activo':''}" onclick="cambiarTabCSF('${tab}')">${label}</button>`
}

async function cambiarTabCSF(tab) {
  _csfTab = tab
  qsa('.tab-btn').forEach(b => b.classList.toggle('tab-activo', b.textContent.includes(tab==='generar'?'Generar':tab==='seguimiento'?'Seguimiento':'Historial')))
...
async function renderGenerador() {
  ...
  const hoy  = new Date()
  // Mes de vigencia = mes actual + 1 o + 2 según día
  const mesVig = hoy.getDate() <= 5
    ? { mes: hoy.getMonth() + 1, anio: hoy.getFullYear() }
    : { mes: hoy.getMonth() + 2 > 12 ? 1 : hoy.getMonth() + 2, anio: hoy.getMonth() + 2 > 12 ? hoy.getFullYear() + 1 : hoy.getFullYear() }
    ...
    <div class="card gap3" style="margin-bottom:1rem">
      ...
      <div class="g3">
        <div class="campo">
          ...
          <div style="font-weight:600;color:var(--verde)">${meses[ref.mes-1]} ${ref.anio}</div>
        ...
        <div class="campo">
          ...
          <div style="font-weight:600;color:var(--verde)">${meses[mesVig.mes-1]} ${mesVig.anio}</div>
...
async function generarBorradorCSF() {
  ...
  // Servicios del mes de referencia
  ...
  // Datos del período de referencia
  ...
  const puntosProcesados = (puntos||[]).map(p => {
    ...
    // Nivel Excel por tipo de delito
    const nivelesExcel = {
      ...
      contrabando:        nivelDesdeDelito('casos',               incautPunto.filter(i=>i.tipo_especie==='fardos_ropa'||i.tipo_especie==='cigarrillos'||i.tipo_especie==='fitozoosanitario').length),
    ...
    // Turno: basado en observaciones nocturnas
  ...
  // Generar todas las visitas ordenadas
...
function htmlBorrador(d) {
  ...
  <div class="card" style="border:2px solid var(--verde);padding:0;overflow:hidden;margin-bottom:1rem">
    ...
    <div style="background:#1A843F;color:#fff;padding:.4rem .85rem;font-size:.74rem;font-weight:700">
      I. NIVELES DE CRITICIDAD POR SECTOR (conforme análisis mes anterior)
    </div>
    <table style="width:100%;border-collapse:collapse;font-size:.72rem">
      <thead>
        <tr style="background:#04742C;color:#fff">
          <th style="padding:.35rem .6rem;text-align:left;width:60px">N°/Sector</th>
    ...
    <!-- SECCIÓN III -->
    <div style="background:#1A843F;color:#fff;padding:.4rem .85rem;font-size:.74rem;font-weight:700">
      III. LUGARES A PATRULLAR SEGÚN DEMANDA OPERATIVA FRONTERIZA (O.G. 3020)
    </div>
    ...
    <div style="background:#F0F9F3;padding:.75rem .85rem;font-size:.72rem;line-height:1.6">
      ...
      <strong>C.</strong> El Jefe de Patrulla es responsable de documentar el cumplimiento de los lineamientos de la presente CSF. Los registros deben ser precisos y reflejar fielmente las acciones ejecutadas.<br>
  ...
  <div style="display:flex;gap:.75rem;flex-wrap:wrap">
    ...
    <button class="btn btn-secundario" onclick="exportarCSFPDF()">↓ Exportar PDF</button>
    ...
    <div class="card" style="border-left:4px solid var(--verde);margin-bottom:1rem">
      <div style="display:flex;justify-content:space-between;align-items:start">
        <div>
          ...
          <div style="font-size:.72rem;color:var(--muted)">${formatFechaCorta(csf.fecha_vigencia_inicio)} → ${formatFechaCorta(csf.fecha_vigencia_fin)} · 30 días</div>
        </div>
      ...
      <table style="width:100%;border-collapse:collapse;font-size:.72rem">
        ...
        <tbody>
          ${(visitas||[]).map((v,i) => {
            ...
            const esInc     = esPasada && v.estado !== 'ejecutada'
...
async function exportarCSFPDF() {
  toast('Exportación PDF disponible en la versión completa', 'ok')
}
