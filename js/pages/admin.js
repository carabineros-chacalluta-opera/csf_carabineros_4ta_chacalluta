...
async function renderAdmin() {
  ...
  await cambiarTabAdmin(_adminTab)
}
...
async function adminPuntos() {
  ...
  if (!cuartelId) { el('admin-contenido').innerHTML = htmlSinCuartelSeleccionado('los puntos territoriales'); return }
  const {data:puntos}=await APP.sb.from('puntos_territoriales').select('*').eq('cuartel_id',cuartelId).order('tipo').order('nombre')
...
async function adminPersonal(){
  const cuartelId = APP.cuartelActivo()?.id
  if (!cuartelId) { el('admin-contenido').innerHTML = htmlSinCuartelSeleccionado('el personal del cuartel'); return }
  const {data:personal}=await APP.sb.from('personal_cuartel').select('*').eq('cuartel_id',cuartelId).order('codigo_funcionario')
  el('admin-contenido').innerHTML=`
    <div class="card">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem">
...
// AUTH-01: sin Supabase Auth — el admin gestiona todo desde aquí.

const _ROL_LABELS = {
  comisario:'Comisario', administrador:'Administrador',
  digitador:'Digitador', validador:'Validador'
}

async function adminUsuarios() {
  const { data: usuarios } = await APP.sb
    .from('usuarios')
  ...
  el('admin-contenido').innerHTML = `
    <div class="card">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem">
        ...
        <button class="btn btn-primario btn-sm" onclick="modalUsuario(null)">+ Nuevo usuario</button>
      </div>
      <div style="overflow-x:auto">
        <table style="width:100%;border-collapse:collapse;font-size:.78rem;min-width:680px">
          <thead><tr style="background:#f0f0f2">
            <th style="padding:.35rem .6rem;text-align:left">Nombre</th>
            <th style="padding:.35rem .6rem;text-align:left;width:100px">Usuario</th>
            <th style="padding:.35rem .6rem;text-align:left;width:100px">Rol</th>
            <th style="padding:.35rem .6rem;text-align:left">Cuartel</th>
            <th style="padding:.35rem .6rem;text-align:center;width:60px">Activo</th>
            <th style="padding:.35rem .6rem;text-align:center;width:180px">Acciones</th>
          </tr></thead>
          <tbody>
            ${(usuarios || []).map((u, i) => `
              <tr style="${i%2===0?'background:#fafafa':''}">
                <td style="padding:.35rem .6rem;font-weight:600">${u.nombre || '—'}
                  ${u.email?`<br><span style="font-size:.68rem;color:var(--muted);font-weight:400">${u.email}</span>`:''}
                </td>
                <td style="padding:.35rem .6rem;font-family:var(--font-mono);font-size:.75rem">${u.username || '<em style="color:var(--muted)">sin usuario</em>'}</td>
                <td style="padding:.35rem .6rem"><span class="badge badge-${u.rol}">${(_ROL_LABELS[u.rol]||u.rol).toUpperCase()}</span></td>
                <td style="padding:.35rem .6rem;font-size:.72rem">${u.cuartel?.nombre?.replace(' (F)','') || '<em style="color:var(--muted)">Sin cuartel</em>'}</td>
                ...
                <td style="padding:.35rem .5rem;text-align:center">
                  <div style="display:flex;gap:.3rem;justify-content:center;flex-wrap:wrap">
                    <button class="btn btn-sm btn-secundario" onclick="modalUsuario('${u.id}')">Editar</button>
                    <button class="btn btn-sm" style="background:#FEF3E2;color:#7B3F00;border:1px solid #F5CBA7;font-size:.72rem" onclick="modalCambiarPassword('${u.id}','${(u.nombre||u.username||'').replace(/'/g,'&apos;')}')">Clave</button>
                    ${u.id !== APP.perfil?.id
                      ? `<button class="btn btn-sm" style="background:#FDEDEC;color:#C0392B;border:1px solid #F1948A;font-size:.72rem" onclick="eliminarUsuario('${u.id}','${(u.nombre||u.username||'').replace(/'/g,'&apos;')}')">Eliminar</button>`
                      : '<span style="font-size:.68rem;color:var(--muted)">(tú)</span>'}
                  </div>
                </td>
              </tr>`).join('')}
            ${!usuarios?.length ? '<tr><td colspan="6" style="padding:1.5rem;text-align:center;color:var(--muted)">Sin usuarios registrados</td></tr>' : ''}
          </tbody>
        </table>
      </div>
    </div>
    <div id="modal-usuario"  class="modal" style="display:none"><div class="modal-box" style="max-width:520px"><div id="form-usuario-contenido"></div></div></div>
    <div id="modal-password" class="modal" style="display:none"><div class="modal-box" style="max-width:400px"><div id="form-password-contenido"></div></div></div>`
}

async function modalUsuario(id) {
  let u = null
  if (id) {
    const { data } = await APP.sb.from('usuarios').select('*').eq('id', id).single()
    u = data
  }
  const { data: cuarteles } = await APP.sb.from('cuarteles').select('id,nombre').eq('activo', true).order('nombre')

  el('form-usuario-contenido').innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem">
      <div class="modal-titulo">${u ? 'Editar usuario' : 'Nuevo usuario'}</div>
      <button onclick="el('modal-usuario').style.display='none'" class="btn-cerrar">✕</button>
    </div>
    <input type="hidden" id="usr-id" value="${u?.id || ''}"/>
    <div class="g2">
      <div class="campo">
        <label>Nombre completo <span style="color:var(--rojo)">*</span></label>
        <input id="usr-nombre" type="text" value="${u?.nombre || ''}" placeholder="Ej: Juan Pérez" autocomplete="off"/>
      </div>
      <div class="campo">
        <label>Nombre de usuario <span style="color:var(--rojo)">*</span></label>
        <input id="usr-username" type="text" value="${u?.username || ''}" placeholder="Ej: jperez" autocomplete="off"
          style="font-family:var(--font-mono)"/>
        <span style="font-size:.68rem;color:var(--muted)">Sin espacios ni mayúsculas. Ej: jperez, admin01</span>
      </div>
      <div class="campo">
        <label>Email (referencia)</label>
        <input id="usr-email" type="email" value="${u?.email || ''}" placeholder="opcional@carabineros.cl" autocomplete="off"/>
      </div>
      <div class="campo">
        <label>Rol <span style="color:var(--rojo)">*</span></label>
        <select id="usr-rol">
          ${Object.entries(_ROL_LABELS).map(([v,l]) =>
            `<option value="${v}" ${u?.rol===v?'selected':''}>${l}</option>`
          ).join('')}
        </select>
      </div>
      <div class="campo" style="grid-column:1/-1">
        <label>Cuartel asignado</label>
        <select id="usr-cuartel">
          <option value="">— Sin cuartel —</option>
          ${(cuarteles || []).map(c =>
            `<option value="${c.id}" ${u?.cuartel_id===c.id?'selected':''}>${c.nombre.replace(' (F)','')}</option>`
          ).join('')}
        </select>
      </div>
      ${!u ? `
      <div class="campo">
        ...
        <input id="usr-password" type="password" placeholder="Mínimo 6 caracteres" autocomplete="new-password"/>
      </div>
      <div class="campo">
        <label>Confirmar contraseña <span style="color:var(--rojo)">*</span></label>
        <input id="usr-password2" type="password" placeholder="Repetir contraseña" autocomplete="new-password"/>
      </div>` : `
      <div style="grid-column:1/-1;background:#EBF3FB;border:1px solid #90CAF9;border-radius:6px;padding:.55rem .85rem;font-size:.75rem;color:#1565C0">
        Para cambiar la contraseña use el botón <strong>Clave</strong> en la lista de usuarios.
      </div>`}
    </div>
    <div style="display:flex;gap:.75rem;margin-top:1.25rem">
      <button class="btn btn-primario" onclick="guardarUsuario()">Guardar</button>
      <button class="btn btn-ghost"    onclick="el('modal-usuario').style.display='none'">Cancelar</button>
    </div>`
  el('modal-usuario').style.display = 'flex'
}

async function guardarUsuario() {
  const id        = el('usr-id')?.value?.trim()
  const nombre    = el('usr-nombre')?.value?.trim()
  const username  = el('usr-username')?.value?.trim().toLowerCase()
  const email     = el('usr-email')?.value?.trim() || null
  const rol       = el('usr-rol')?.value
  const cuartelId = el('usr-cuartel')?.value || null

  if (!nombre)   { toast('Ingrese el nombre completo', 'err'); return }
  if (!username) { toast('Ingrese el nombre de usuario', 'err'); return }
  if (/\s/.test(username)) { toast('El usuario no puede tener espacios', 'err'); return }

  const dat = { nombre, username, email, rol, cuartel_id: cuartelId }

  if (!id) {
    const pass  = el('usr-password')?.value
    const pass2 = el('usr-password2')?.value
    ...
    dat.password_hash = await hashPassword(pass)
    dat.activo = true
  }

  const { error } = id
    ? await APP.sb.from('usuarios').update(dat).eq('id', id)
    : await APP.sb.from('usuarios').insert(dat)

  if (error) {
    if (error.message?.includes('usuarios_username_uq') || error.message?.includes('username'))
      toast('El nombre de usuario "' + username + '" ya está en uso', 'err')
    else
      toast('Error al guardar: ' + error.message, 'err')
    return
  }

  toast(id ? 'Usuario actualizado' : 'Usuario creado correctamente', 'ok')
  el('modal-usuario').style.display = 'none'
  await adminUsuarios()
}

function modalCambiarPassword(userId, nombre) {
  el('form-password-contenido').innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem">
      <div class="modal-titulo">Cambiar contraseña</div>
      <button onclick="el('modal-password').style.display='none'" class="btn-cerrar">✕</button>
    </div>
    <div style="font-size:.8rem;color:var(--muted);margin-bottom:1rem">Usuario: <strong>${nombre}</strong></div>
    <input type="hidden" id="cpw-id" value="${userId}"/>
    <div class="campo" style="margin-bottom:.75rem">
      <label>Nueva contraseña</label>
      <input id="cpw-nueva" type="password" placeholder="Mínimo 6 caracteres" autocomplete="new-password"/>
    </div>
    <div class="campo" style="margin-bottom:1.25rem">
      <label>Confirmar contraseña</label>
      <input id="cpw-confirmar" type="password" placeholder="Repetir contraseña" autocomplete="new-password"/>
    </div>
    <div style="display:flex;gap:.5rem">
      <button class="btn btn-primario" onclick="ejecutarCambioPassword()">Guardar contraseña</button>
      <button class="btn btn-ghost"    onclick="el('modal-password').style.display='none'">Cancelar</button>
    </div>`
  el('modal-password').style.display = 'flex'
}

async function ejecutarCambioPassword() {
  const id        = el('cpw-id')?.value
  const nueva     = el('cpw-nueva')?.value
  const confirmar = el('cpw-confirmar')?.value

  if (!nueva)             { toast('Ingrese la nueva contraseña', 'err'); return }
  ...
  const { error } = await APP.sb.from('usuarios').update({ password_hash: hash }).eq('id', id)
  if (error) { toast('Error: ' + error.message, 'err'); return }
  toast('Contraseña actualizada correctamente', 'ok')
  el('modal-password').style.display = 'none'
}

async function eliminarUsuario(id, nombre) {
  if (!confirm(`¿Eliminar al usuario "${nombre}"?\n\nEsta acción no se puede deshacer.`)) return
  const { error } = await APP.sb.from('usuarios').delete().eq('id', id)
...
async function adminDenominadores(){
  const cuartelId=APP.cuartelActivo()?.id
  if(!cuartelId){el('admin-contenido').innerHTML=htmlSinCuartelSeleccionado('los denominadores IDFI');return}
  const {data:cfg}=await APP.sb.from('config_cuartel').select('*').eq('cuartel_id',cuartelId).maybeSingle()
  el('admin-contenido').innerHTML=`<div class="card">
...
async function adminFirmas(){
  const cuartelId=APP.cuartelActivo()?.id
  if(!cuartelId){el('admin-contenido').innerHTML=htmlSinCuartelSeleccionado('las firmas del cuartel');return}
