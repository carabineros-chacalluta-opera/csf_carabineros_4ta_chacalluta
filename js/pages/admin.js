// ============================================================
// SISTEMA CSF OPERATIVA — admin.js  v1.1
// CORRECCIONES:
//   B2  — denominadores IDFI persisten en tabla config_cuartel
//   B8  — tabs usan data-tab attribute (no textContent.includes)
//   S2  — adminUsuarios filtra por cuartel del administrador
// ============================================================
...
// B8: data-tab en lugar de textContent
...
// ── USUARIOS ─────────────────────────────────────────────────
// S2: El RLS de usuarios solo permite ver el propio registro.
// Para que el comisario/admin vea los usuarios del cuartel se usa
// una función SECURITY DEFINER en Supabase (ver 03_rls.sql).
// Mientras tanto, mostramos los datos disponibles + instrucciones.
  ...
  el('admin-contenido').innerHTML = `
    <div class="card">
      ...
      <div style="font-size:.75rem;color:var(--muted);background:var(--azul-cl);border:1px solid var(--azul);border-radius:6px;padding:.65rem .85rem;margin-bottom:1rem">
        ℹ Por seguridad, cada usuario solo puede ver su propio perfil. Para ver todos los usuarios del cuartel,
        el administrador de Supabase debe otorgar permisos adicionales (ver README → Sección seguridad).
      </div>
          ...
          ${(usuarios||[]).map((u,i) => `
            <tr style="${i%2===0?'background:#fafafa':''};border-bottom:1px solid var(--border)">
              ...
              <td style="padding:.35rem .6rem">
                <span class="badge badge-${u.rol}" style="${u.rol==='validador'?'background:#EBF3FB;color:#1565C0;border:1px solid #90CAF9':''}">
      ...
      <div style="margin-top:1rem;padding-top:1rem;border-top:1px solid var(--border)">
        ...
        <ol style="font-size:.73rem;color:var(--muted);padding-left:1.25rem;line-height:2">
          <li>Supabase Dashboard → <strong>Authentication → Users → Invite user</strong></li>
          <li>Ingresar el email del nuevo usuario</li>
          <li>SQL Editor:
            <code style="display:block;background:#f0f0f2;padding:4px 8px;border-radius:4px;margin-top:4px;font-size:.7rem">
              INSERT INTO usuarios (id, email, cuartel_id, rol)<br>
              VALUES ('UUID_USUARIO', 'email@ejemplo.com', '${APP.cuartel?.id||'UUID_CUARTEL'}', 'digitador');<br>
              <span style="color:#1565C0">-- Para Subprefecto Fronterizo usar rol: 'validador'<br>
              -- El validador NO necesita cuartel_id (puede ser NULL)</span>
            </code>
          </li>
        </ol>
      </div>
...
async function adminDenominadores() {
  ...
  el('admin-contenido').innerHTML = `
    <div class="card">
      ...
      <div class="sec-titulo" style="font-size:.82rem">Total funcionarios del cuartel (denominador IDFI general)</div>
      <div class="campo-inline">
      ...
      ${cfg?.updated_at ? `<div style="font-size:.7rem;color:var(--muted);margin-top:1rem">Última actualización: ${formatFechaCorta(cfg.updated_at.split('T')[0])}</div>` : ''}
    </div>`
}

// B2: guardar denominadores en Supabase
async function guardarConfigIDFI() {
  ...
  if (error) {
    ...
    console.error('guardarConfigIDFI:', error)
  } else {
...
// ── VALIDADORES (Subprefecto Fronterizo) ───────────────────────
async function adminValidadores() {
  // Traer usuarios con rol validador
  ...
  el('admin-contenido').innerHTML = `
    <div class="card">
      ...
      <div style="font-size:.78rem;color:#1565C0;background:#EBF3FB;border:1px solid #90CAF9;
          border-radius:6px;padding:.65rem .85rem;margin-bottom:1rem;line-height:1.6">
        🔍 Los usuarios con rol <strong>validador</strong> tienen acceso exclusivo a la pantalla
        de CSF en revisión. Pueden ver todas las cartas pendientes, editar horarios de visita
        y aprobar masivamente con su firma digital.
        No tienen acceso a servicios, dashboard ni administración.
      </div>
      ...
      <table style="width:100%;border-collapse:collapse;font-size:.78rem;margin-bottom:1.25rem">
        ...
        <tbody>
          ...
          ${!validadores?.length ? `
          <tr><td colspan="4" style="padding:1.5rem;text-align:center;color:var(--muted)">
            Sin validadores registrados
          </td></tr>` : ''}
        </tbody>
      </table>

      <!-- Formulario para crear nuevo validador -->
      <div style="border-top:1px solid var(--border);padding-top:1rem">
        <div class="sec-titulo" style="font-size:.82rem;margin-bottom:.75rem">
          Registrar nuevo validador
        </div>
        <div style="font-size:.75rem;color:var(--muted);margin-bottom:.75rem;line-height:1.6">
          El usuario debe existir previamente en <strong>Supabase → Authentication → Users</strong>.
        ...
        <button class="btn btn-primario" onclick="crearValidador()">
          + Registrar validador
        </button>
      </div>
      ...
      <div style="margin-top:1.25rem;padding-top:1rem;border-top:1px solid var(--border)">
        ...
        <code style="display:block;background:#f0f0f2;padding:6px 10px;border-radius:4px;margin-top:4px;font-size:.7rem;line-height:1.7">
          INSERT INTO usuarios (id, email, cuartel_id, rol, activo)<br>
          VALUES ('UUID_USUARIO', 'email@carabineros.cl', NULL, 'validador', true);
        </code>
      </div>
    </div>`
}
...
async function crearValidador() {
  ...
  const btn = document.querySelector('#admin-contenido .btn-primario')
  ...
  if (error) {
    ...
    if (btn) { btn.disabled = false; btn.textContent = '+ Registrar validador' }
    return
  }
...
async function revocarValidador(id, email) {
  if (!confirm(`¿Revocar acceso de validador a ${email}? El usuario quedará inactivo.`)) return
