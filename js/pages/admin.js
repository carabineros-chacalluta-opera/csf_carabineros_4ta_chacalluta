...
// ── ADMIN FIRMAS (Mejoras 3 y 4) ─────────────────────────────
// Permite configurar nombre, grado, cargo e imagen de firma
// para el comisario y el validador de cada cuartel.
// La firma se guarda como base64 en tabla config_firmas.
...
async function adminFirmas() {
  ...
  const { data: firmas } = await APP.sb.from('config_firmas')
    .select('*').eq('cuartel_id', cuartelId)
  ...
  el('admin-contenido').innerHTML = `
    ...
    <div class="card" style="margin-top:1rem">
      <div class="sec-titulo" style="margin-bottom:.75rem">Vista previa del pie de firma</div>
      <div style="display:grid;grid-template-columns:1fr 1fr;border:1px solid #ddd;min-height:100px;max-width:700px;margin:0 auto">

        <!-- Comisario preview -->
        <div id="preview-comisario" style="border-right:1px solid #ddd;padding:1rem;text-align:center">
          ${firmaComisario.firma_b64
            ? `<img src="${firmaComisario.firma_b64}" style="height:55px;max-width:180px;object-fit:contain;display:block;margin:0 auto .3rem"/>`
            : '<div style="height:55px;display:flex;align-items:center;justify-content:center;color:#ccc;font-size:.72rem">Sin firma cargada</div>'}
          <div style="border-top:2px solid #000;padding-top:.3rem;margin-top:.2rem">
            <div style="font-size:.76rem;font-weight:700">${firmaComisario.nombre || '—'}</div>
            <div style="font-size:.7rem">${firmaComisario.grado || '—'}</div>
            <div style="font-size:.7rem;font-weight:700">${firmaComisario.cargo || 'COMISARIO'}</div>
          </div>
        </div>

        <!-- Validador preview -->
        <div id="preview-validador" style="padding:1rem;text-align:center">
          ${firmaValidador.firma_b64
            ? `<img src="${firmaValidador.firma_b64}" style="height:55px;max-width:180px;object-fit:contain;display:block;margin:0 auto .3rem"/>`
            : '<div style="height:55px;display:flex;align-items:center;justify-content:center;color:#ccc;font-size:.72rem">Sin firma cargada</div>'}
          <div style="border-top:2px solid #000;padding-top:.3rem;margin-top:.2rem">
            <div style="font-size:.76rem;font-weight:700">${firmaValidador.nombre || '—'}</div>
            <div style="font-size:.7rem">${firmaValidador.grado || '—'}</div>
            <div style="font-size:.7rem;font-weight:700">${firmaValidador.cargo || 'SUBPREFECTO FRONTERIZO'}</div>
          </div>
        </div>
...
function _htmlCardFirma(rol, titulo, firma) {
  return `
    <div class="card">
      <div class="sec-titulo" style="margin-bottom:1rem">${titulo}</div>

      <div class="campo" style="margin-bottom:.75rem">
        <label>Nombre completo</label>
        <input id="firma-${rol}-nombre" type="text"
          value="${firma.nombre || ''}"
          placeholder="Ej: NILO A. MORALES RIQUELME"
          style="text-transform:uppercase"/>
      </div>
      <div class="campo" style="margin-bottom:.75rem">
        <label>Grado</label>
        <input id="firma-${rol}-grado" type="text"
          value="${firma.grado || ''}"
          placeholder="Ej: Teniente Coronel de Carabineros"/>
      </div>
      <div class="campo" style="margin-bottom:.75rem">
        <label>Cargo</label>
        <input id="firma-${rol}-cargo" type="text"
          value="${firma.cargo || ''}"
          placeholder="${rol === 'comisario' ? 'COMISARIO' : 'SUBPREFECTO FRONTERIZO'}"
          style="text-transform:uppercase"/>
      </div>

      <div class="campo" style="margin-bottom:.75rem">
        <label>Imagen de firma
          <span style="font-size:.68rem;color:var(--muted)">(PNG fondo transparente recomendado, max 300KB)</span>
        </label>
        <input type="file" id="firma-${rol}-archivo" accept="image/png,image/jpeg,image/jpg"
          onchange="previsualizarFirmaAdmin('${rol}', this)"/>
        <div id="firma-${rol}-preview" style="margin-top:.5rem;text-align:center;min-height:60px;border:1px dashed var(--border);border-radius:6px;padding:.5rem;display:flex;align-items:center;justify-content:center">
          ${firma.firma_b64
            ? `<img src="${firma.firma_b64}" style="max-height:60px;max-width:200px;object-fit:contain"/>`
            : '<span style="font-size:.72rem;color:var(--muted)">Sin imagen</span>'}
        </div>
      </div>

      <div style="display:flex;gap:.5rem;margin-top:.25rem">
        <button class="btn btn-primario btn-sm" onclick="guardarFirmaAdmin('${rol}')">
          Guardar ${titulo}
        </button>
        ${firma.firma_b64 ? `
        <button class="btn btn-sm" style="background:#fdecea;color:var(--rojo);border:1px solid #f5c6c6"
          onclick="eliminarFirmaImagen('${rol}')">
          Borrar imagen
        </button>` : ''}
      </div>
    </div>`
...
function previsualizarFirmaAdmin(rol, input) {
  const file = input?.files?.[0]
  if (!file) return
  if (file.size > 400 * 1024) {
    toast('Imagen demasiado grande (max 300KB). Reduce el tamano.', 'warn')
    input.value = ''
    return
  }
  const reader = new FileReader()
  reader.onload = e => {
    const prev = el(`firma-${rol}-preview`)
    if (prev) prev.innerHTML = `<img src="${e.target.result}" style="max-height:60px;max-width:200px;object-fit:contain"/>`
  }
  reader.readAsDataURL(file)
}

async function guardarFirmaAdmin(rol) {
  const cuartelId = APP.cuartelActivo()?.id
  if (!cuartelId) { toast('Sin cuartel activo', 'err'); return }

  const nombre = el(`firma-${rol}-nombre`)?.value?.trim().toUpperCase()
  const grado  = el(`firma-${rol}-grado`)?.value?.trim()
  const cargo  = el(`firma-${rol}-cargo`)?.value?.trim().toUpperCase()

  if (!nombre || !grado || !cargo) {
    toast('Completa nombre, grado y cargo', 'err')
    return
  }

  // Leer imagen si se selecciono una nueva
  let firma_b64 = null
  const archivo = el(`firma-${rol}-archivo`)
  if (archivo?.files?.[0]) {
    firma_b64 = await new Promise(resolve => {
      const reader = new FileReader()
  ...
  const datos = { cuartel_id: cuartelId, rol, nombre, grado, cargo, updated_at: new Date().toISOString() }
  if (firma_b64) datos.firma_b64 = firma_b64

  const { error } = await APP.sb.from('config_firmas').upsert(datos, { onConflict: 'cuartel_id,rol' })

  if (error) { toast('Error: ' + error.message, 'err'); return }

  // Limpiar cache de firma en csf.js
  if (typeof _firmaComisarioCache !== 'undefined') {
    _firmaComisarioCache = {}
  }

  toast(`Firma de ${rol} guardada correctamente`, 'ok')
  await adminFirmas()
}

async function eliminarFirmaImagen(rol) {
  if (!confirm('Borrar la imagen de firma de ' + rol + '?')) return
  const cuartelId = APP.cuartelActivo()?.id
  const { error } = await APP.sb.from('config_firmas')
    .update({ firma_b64: null, updated_at: new Date().toISOString() })
    .eq('cuartel_id', cuartelId).eq('rol', rol)
  if (error) { toast('Error: ' + error.message, 'err'); return }
  if (typeof _firmaComisarioCache !== 'undefined') _firmaComisarioCache = {}
