...
// Módulo independiente de registro de capacitaciones
// ============================================================
    ...
    <div class="container">
      ...
      <div class="card filtros-card" style="margin-bottom:1rem">
        <div class="g3">
          <div class="campo">
            <label>Desde</label>
            <input type="date" id="cap-desde" value="${ini}"/>
          </div>
          <div class="campo">
            <label>Hasta</label>
            <input type="date" id="cap-hasta" value="${hoy}"/>
          </div>
          ${puedeVerTodos ? `
          <div class="campo">
            <label>Cuartel</label>
...
async function consultarCapacitaciones() {
  const desde = el('cap-desde')?.value
  const hasta = el('cap-hasta')?.value
  ...
  let q = APP.sb.from('capacitaciones')
  ...
  if (!caps?.length) {
    zona.innerHTML = '<div class="card" style="text-align:center;padding:2rem;color:var(--muted)">Sin capacitaciones en el período</div>'
    return
  }
  ...
  zona.innerHTML = `
    <div class="card">
      <table style="width:100%;border-collapse:collapse;font-size:.8rem">
        ...
        <tbody>
          ${caps.map((c,i)=>`
            <tr style="${i%2===0?'background:var(--surface-2)':''}">
              ...
              <td style="padding:.35rem .6rem">
                <span style="font-size:.7rem;font-weight:700;padding:1px 6px;border-radius:3px;
                  background:${c.tipo==='institucional'?'#e8f5ea':'#e8f0fe'};
                  color:${c.tipo==='institucional'?'#1A843F':'#3730a3'}">
                  ${c.tipo==='institucional'?'Institucional':'Extrainst.'}
                </span>
              </td>
              ...
              ${!APP.esDigitador() ? `
              <td style="padding:.35rem .6rem">
                <button class="btn btn-sm btn-secundario" onclick="eliminarCapacitacion('${c.id}')">✕</button>
              </td>` : ''}
            </tr>`).join('')}
        </tbody>
      </table>
  ...
  el('form-capacitacion-contenido').innerHTML = `
    <div class="g2">
      ...
      <div class="campo">
        ...
        <input type="date" id="cap-fi" value="${hoyISO()}"/>
      </div>
      <div class="campo">
        <label>Fecha término</label>
        <input type="date" id="cap-ft" value="${hoyISO()}"/>
      </div>
    </div>
    <div class="campo" style="margin-top:.5rem">
      <label>Temática tratada</label>
      <textarea id="cap-tematica" rows="2" placeholder="Ej: Mal agudo de montaña, Uso GPS, Rheinmetall..."></textarea>
    </div>
    <div class="campo" style="margin-top:.5rem">
      <label>Organismo o instructor que realiza la capacitación</label>
      <input type="text" id="cap-organismo" placeholder="Ej: Carabineros de Chile, Cabo 1ro. Juan Pérez..."/>
    </div>
    <div class="campo" style="margin-top:.5rem">
      <label>Cantidad de personal capacitado</label>
...
async function guardarCapacitacion() {
  ...
  const fi = el('cap-fi')?.value
  const ft = el('cap-ft')?.value
  const tematica = el('cap-tematica')?.value?.trim()
  const organismo = el('cap-organismo')?.value?.trim()
  ...
  const { error } = await APP.sb.from('capacitaciones').insert({
    cuartel_id:       cuartelId,
    tipo:             el('cap-tipo')?.value,
    fecha_inicio:     fi,
    fecha_termino:    ft || fi,
    tematica,
    organismo:        organismo || null,
    cantidad_personal: personal,
    registrado_por:   APP.perfil.id,
  })
